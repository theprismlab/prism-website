...
/**
 * Three.js scene composable for bubble visualizations.
 ...
 *
 * Usage:
 *   const bubble = useBubbleScene(canvasEl, overrides);
 *   onMounted(async () => {
 *       // load data, create meshes...
 *       bubble.scene.add(mesh);
 *       bubble.onAnimate(myCallback);
 *       bubble.render();
 *       bubble.startAnimation();
 *       bubble.onRebuild(() => { // recreate meshes after resize });
 *   });
 */
export function useBubbleScene(canvasEl, configOverrides = {}) {
