...
function updateSpheres(t) {
  spheres.forEach(sphere => {
    const { initialX, initialZ, speed, waveOffset, maxRadius, rainbowColorScale } = sphere.userData;

    // Bubble up slowly
    sphere.position.y += speed;

    // Apply continuous horizontal movement
    const horizontalSpeed = 0.01; // Speed of horizontal movement
    sphere.position.x += horizontalSpeed; // Continuous horizontal movement

    // Apply reduced vertical wave motion
    const verticalAmplitude = 0.2; // Reduced amplitude for less vertical variation
    sphere.position.y += Math.sin(t * 0.001 + waveOffset) * verticalAmplitude; // Vertical wave

    // Depth (z) remains fixed
    ...
    // Reset position if sphere floats off the page
    if (sphere.position.y > 15) { // Float off the page before resetting
      sphere.position.y = 0.1; // Reset near heatmap
      sphere.position.x = initialX; // Reset horizontal position
...
//     // Apply wave motion with reduced horizontal movement
//     const horizontalAmplitude = 0.5; // Reduce amplitude for less horizontal movement
//     sphere.position.x = initialX + Math.sin(t * 0.001 + waveOffset) * horizontalAmplitude; // Horizontal wave
//     sphere.position.z = initialZ + Math.cos(t * 0.001 + waveOffset) * horizontalAmplitude; // Depth wave
