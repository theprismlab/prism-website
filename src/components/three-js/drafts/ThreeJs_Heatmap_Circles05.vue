...
function initSpheres() {
  ...
  const N = 300; // Number of spheres
  const heatmapColorScale = d3.scaleSequential(d3.interpolateViridis).domain([0, 10]); // Heatmap color scale
  ...
  for (let i = 0; i < N; i++) {
    ...
    const material = new THREE.MeshStandardMaterial({
      color: new THREE.Color(heatmapColorScale(0)), // Start with heatmap color
      transparent: true,
    ...
    // Initial position
    const x = Math.random() * 20 - 10; // Random x within heatmap bounds
    const y = Math.random() * 2; // Start near the heatmap
    const z = Math.random() * 20 - 10; // Random z within heatmap bounds
    ...
    sphere.userData = {
      ...
      colorScale: heatmapColorScale // Start with heatmap color scale
    };
...
function updateSpheres(t) {
  ...
  spheres.forEach(sphere => {
    const { initialX, initialZ, speed, waveOffset } = sphere.userData;
    ...
    const heatmapColor = heatmapColorScale(normalizedHeight * 10); // Map normalized height to heatmap color
    const rainbowColor = rainbowColorScale(normalizedHeight * 10); // Map normalized height to rainbow color

    // Blend the two colors based on height
    const blendedColor = d3.interpolateRgb(heatmapColor, rainbowColor)(normalizedHeight);
    sphere.material.color.set(blendedColor);

    // Reset position if sphere moves too far up
    if (sphere.position.y > 10) {
      sphere.position.y = Math.random() * 2; // Reset near heatmap
...
onMounted(async () => {
  ...
  initThreeJs();
  await loadHeatmapData(); // Assuming heatmap logic is already implemented
  initSpheres();
  animate();
