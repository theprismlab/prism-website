...
//     const { initialX, initialZ, speed, maxRadius, rainbowColorScale } = sphere.userData;

//     // Float upward slowly like balloons
//     sphere.position.y += speed * 0.5; // Reduce speed for slower upward movement

//     // Apply gentle vertical wave motion
//     const verticalAmplitude = 0.1; // Small amplitude for gentle floating effect
//     sphere.position.y += Math.sin(t * 0.001) * verticalAmplitude; // Vertical wave

//     // Keep horizontal and depth positions fixed
//     sphere.position.x = initialX;
//     sphere.position.z = initialZ;

//     // Calculate distance from origin
//     const distance = Math.sqrt(
//       Math.pow(sphere.position.x - initialX, 2) +
//       Math.pow(sphere.position.y - 0.1, 2) + // Origin y is slightly above heatmap
//       Math.pow(sphere.position.z - initialZ, 2)
//     );

//     // Set radius based on distance
//     const normalizedDistance = Math.min(distance / 10, 1); // Normalize distance to [0, 1]
//     const newRadius = normalizedDistance * maxRadius; // Scale radius based on normalized distance

//     // Update sphere geometry with new radius
//     if (sphere.geometry.parameters.radius !== newRadius) {
//       sphere.geometry.dispose(); // Dispose old geometry
//       sphere.geometry = new THREE.SphereGeometry(newRadius, 16, 16); // Update geometry with new radius
//     }
...
//     // Reset position if sphere floats off the screen
//     if (sphere.position.y > 20) { // Float off the screen before resetting
...
function updateSpheres(t) {
  spheres.forEach(sphere => {
    const { initialX, initialZ, speed, maxRadius, waveOffset, rainbowColorScale } = sphere.userData;

    // Float upward slowly
    sphere.position.y += speed * 0.5; // Reduce speed for slower upward movement

    // Apply unique wave motion for each sphere
    const horizontalAmplitude = 2; // Amplitude for horizontal movement
    const depthAmplitude = 1.5; // Amplitude for depth movement
    const waveFrequency = 0.001; // Frequency of the wave motion

    // Horizontal (x) wave motion
    sphere.position.x = initialX + Math.sin(t * waveFrequency + waveOffset) * horizontalAmplitude;

    // Depth (z) wave motion
    sphere.position.z = initialZ + Math.cos(t * waveFrequency + waveOffset) * depthAmplitude;
