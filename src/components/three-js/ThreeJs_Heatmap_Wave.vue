...
function initSpheres(heatmapData) {
  ...
  heatmapData.forEach(d => {
    ...
    sphere.userData = {
      ...
      rainbowColorScale,
      startDelay: Math.random() * 2000 // Random delay up to 2 seconds
    };
...
function updateSpheres(t) {
  spheres.forEach(sphere => {
    const { initialX, initialZ, speed, waveOffset, radius, maxRadius, rainbowColorScale, startDelay } = sphere.userData;

    // Only animate if the delay has passed
    if (t < startDelay) {
      // Keep sphere hidden and at initial state
      sphere.material.opacity = 1;
      sphere.userData.radius = 0;
      sphere.position.y = 1;
      return;
    }
    ...
    if (sphere.position.y > 20) {
      ...
      sphere.userData.startDelay = t + Math.random() * 2000; // New delay for next appearance
    }
  });
}
