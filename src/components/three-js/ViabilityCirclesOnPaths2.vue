...
onMounted(async () => {
    ...
    let data = props.data.filter(d => !isNaN(+d.viability)).sort((a, b) => a.dose - b.dose);
    buildSpheres(data);
    ...
    controls.addEventListener('end', () => {
        ...
        console.log(
            `\n── Camera Config ──\n` +
            `cameraPosition: [${cam.position.x.toFixed(2)}, ${cam.position.y.toFixed(2)}],\n` +
            `cameraDistance: ${cam.position.z.toFixed(2)},\n` +
            `cameraLookAt: [${t.x.toFixed(2)}, ${t.y.toFixed(2)}, ${t.z.toFixed(2)}],\n` +
            `// polar: ${controls.getPolarAngle().toFixed(3)} rad (${THREE.MathUtils.radToDeg(controls.getPolarAngle()).toFixed(1)}°)\n` +
            `// azimuth: ${controls.getAzimuthalAngle().toFixed(3)} rad (${THREE.MathUtils.radToDeg(controls.getAzimuthalAngle()).toFixed(1)}°)\n` +
