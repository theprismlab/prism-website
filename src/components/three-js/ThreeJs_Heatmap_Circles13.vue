...
function updateSpheres(t) {
  spheres.forEach(sphere => {
    ...
    if (sphere.position.y > 20) {
      ...
      sphere.position.x = initialX + Math.random() * 2 - 1; // Slight random horizontal offset
      sphere.position.z = initialZ + Math.random() * 2 - 1; // Slight random depth offset
  ...
  // Ensure continuous flow of spheres
  while (spheres.length < 50) { // Maintain a minimum number of spheres
    const newSphere = createNewSphere(); // Function to create a new sphere
    scene.add(newSphere);
...
function createNewSphere() {
  const initialX = Math.random() * 10 - 5; // Random initial horizontal position
  const initialZ = Math.random() * 10 - 5; // Random initial depth position
  const maxRadius = Math.random() * 0.3 + 0.1; // Random maximum radius
  const speed = Math.random() * 0.02 + 0.01; // Random speed
  const waveOffset = Math.random() * Math.PI * 2; // Random wave offset
  ...
  const sphere = new THREE.Mesh(sphereGeometry, material);
  sphere.position.set(initialX, 0.3, initialZ); // Start near the heatmap
  ...
  sphere.userData = {
    ...
    speed,
    waveOffset,
    rainbowColorScale: d3.scaleSequential(d3.interpolateRainbow).domain([0, 10]) // Rainbow color scale
  };
