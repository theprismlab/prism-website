...
init() {
      ...
      const isBarcode = this.currentSequence === 1 || this.targetSequence === 1;
        // Camera setup
        let camera;
        if (isBarcode) {
        // Calculate bounds to fit all barcode circles
        const aspect = width / height;
        const viewWidth = 30; // Adjust as needed to fit your barcode width
        ...
        camera = new THREE.PerspectiveCamera(70, width / height, 0.01, 40);
        camera.position.z = 10;
        camera.position.y = 1;
  ...
  const gridSize = Math.ceil(Math.sqrt(N));
      ...
      // Use a square grid for the wave
const waveGridSize = Math.ceil(Math.sqrt(N));
