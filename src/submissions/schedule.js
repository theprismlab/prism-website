...
// isValidScreen: optional (screenName, screenType) => { valid, message } lookup, e.g.
// screenStatusStore.isValidScreen. Checks both that this schedule entry IS the type's current
...
// overridden to OPEN.
export function computedStatus(item, isValidScreen) {
  const validation = isValidScreen?.(item.screen_name, item.screen_type);
  console.log('[schedule] computedStatus', {
    screen_type: item.screen_type,
    screen_name: item.screen_name,
    validation,
  });
  if (validation?.valid) return 'OPEN';
  const today = todayET();
...
export function enrichEntry(item, isValidScreen) {
  const status = computedStatus(item, isValidScreen);
  return {
    ...item,
    status,
...
export function enrichedSchedule(isValidScreen) {
  return SCHEDULE.map((item) => enrichEntry(item, isValidScreen));
}
