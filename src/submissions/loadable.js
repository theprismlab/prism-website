// Shared "load once, share in-flight fetches, refresh on demand, surface errors" scaffolding
// for Pinia stores whose state is built from a single cached API fetch. Used by
// active-screen-store.js and window-status-store.js so the load/cache/error bookkeeping only
// lives in one place.

export function loadableState() {
  return { loading: false, loaded: false, error: null, _loadPromise: null };
}
...
// load-once/share-in-flight/error bookkeeping around it.
