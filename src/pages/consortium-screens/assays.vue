        ...
        <v-expansion-panel v-for="item in items" :key="item.id">
          <v-expansion-panel-title>
            ...
            <div class="assay-header">
              <h2 class="prism-text-title-large">{{ item.screen }} - {{ item.screen_full }}</h2>
              <h3 class="prism-text-body-large text-medium-emphasis">{{ item.test_agents }}</h3>
            </div>
            <span class="assay-time-point"> {{ item.time_point }}</span>
            <!-- <v-icon class="category-icon mr-3" size="28">{{ categoryIcons[item.category] }}</v-icon> -->
          </v-expansion-panel-title>
          <v-expansion-panel-text>
            <h4 class="prism-text-title-medium mb-2">{{ item.screen }} Workflow</h4>

            <p class="prism-text-body-large" v-html="item.description"></p>
            <h5 class="prism-text-title-medium mt-4">Dose Scheme</h5>
            <div class="prism-text-body-medium" v-html="item.dose_scheme"></div>
            <h5 class="prism-text-title-medium mt-4">Cell Lines</h5>
            <div class="prism-text-body-medium">
              {{ item.num_cell_lines }}
            </div>
            <v-img :src="imgPath + item.image" />
          </v-expansion-panel-text>
        </v-expansion-panel>
      </v-expansion-panels>
    </app-container>
...
<style scoped>
  ...
  /* Fills remaining space so the time-point sits just before the chevron */
  .assay-header {
    flex: 1 1 0;
    min-width: 0;
  }
