...
export default class ScatterPlot3D {
  ...
  _computeScales(data) {
    ...
    // ── color ─────────────────────────────────────────────────────────────
    const colorScale = d3
      .scaleSequential(this.config.colorInterpolator ?? d3.interpolateYlOrRd)
    ...
    const worldPositions = data.map(
      (d) => new THREE.Vector3(xScale(d.x), yScale(d.y), zScale(d.z)),
    );
  ...
  _buildSpheres(data) {
    ...
    data.forEach((d, i) => {
      ...
      sphere.userData.ox = 0;
      sphere.userData.oy = 0;
      sphere.userData.oz = 0;
    ...
    this._addAnimationCallback((elapsed) => {
      spheres.forEach((s) => {
        ...
        s.userData.floatX =
          basePosition.x + Math.sin(elapsed * floatSpeedX + floatPhaseX) * floatAmplitudeX;
        s.userData.floatY =
          basePosition.y + Math.sin(elapsed * floatSpeed + floatPhase) * floatAmplitude;
    ...
    spheres
      ...
      .forEach((s) =>
        this.barcodeManager.attachSticker(
          s,
          s.userData.radius,
    ...
    for (let i = 0; i < spheres.length; i++) {
      for (let j = i + 1; j < spheres.length; j++) {
        ...
        if (distSq < minDist * minDist && distSq > 1e-6) {
          ...
          si.userData.vx += dx * push;
          si.userData.vy += dy * push;
          si.userData.vz += dz * push;
          sj.userData.vx -= dx * push;
          sj.userData.vy -= dy * push;
          sj.userData.vz -= dz * push;
        }
      }
    }
  }
