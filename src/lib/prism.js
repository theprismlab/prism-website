import { openBlock as n, createElementBlock as s, normalizeClass as u, renderSlot as y, normalizeStyle as W, createElementVNode as l, toDisplayString as o, resolveComponent as D, createBlock as A, resolveDynamicComponent as T, mergeProps as M, withCtx as g, Fragment as v, renderList as k, createTextVNode as _, createCommentVNode as p, createVNode as R, Transition as X, withModifiers as E } from "vue";
const w = {
  red: {
    base: "#f44336",
    50: "#ffebee",
    100: "#ffcdd2",
    200: "#ef9a9a",
    300: "#e57373",
    400: "#ef5350",
    500: "#f44336",
    600: "#e53935",
    700: "#d32f2f",
    800: "#c62828",
    900: "#b71c1c",
    a100: "#ff8a80",
    a200: "#ff5252",
    a400: "#ff1744",
    a700: "#d50000",
    lighten5: "#ffebee",
    lighten4: "#ffcdd2",
    lighten3: "#ef9a9a",
    lighten2: "#e57373",
    lighten1: "#ef5350",
    darken1: "#e53935",
    darken2: "#d32f2f",
    darken3: "#c62828",
    darken4: "#b71c1c",
    accent1: "#ff8a80",
    accent2: "#ff5252",
    accent3: "#ff1744",
    accent4: "#d50000"
  },
  pink: {
    base: "#e91e63",
    50: "#fce4ec",
    100: "#f8bbd0",
    200: "#f48fb1",
    300: "#f06292",
    400: "#ec407a",
    500: "#e91e63",
    600: "#d81b60",
    700: "#c2185b",
    800: "#ad1457",
    900: "#880e4f",
    a100: "#ff80ab",
    a200: "#ff4081",
    a400: "#f50057",
    a700: "#c51162",
    lighten5: "#fce4ec",
    lighten4: "#f8bbd0",
    lighten3: "#f48fb1",
    lighten2: "#f06292",
    lighten1: "#ec407a",
    darken1: "#d81b60",
    darken2: "#c2185b",
    darken3: "#ad1457",
    darken4: "#880e4f",
    accent1: "#ff80ab",
    accent2: "#ff4081",
    accent3: "#f50057",
    accent4: "#c51162"
  },
  purple: {
    base: "#9c27b0",
    50: "#f3e5f5",
    100: "#e1bee7",
    200: "#ce93d8",
    300: "#ba68c8",
    400: "#ab47bc",
    500: "#9c27b0",
    600: "#8e24aa",
    700: "#7b1fa2",
    800: "#6a1b9a",
    900: "#4a148c",
    a100: "#ea80fc",
    a200: "#e040fb",
    a400: "#d500f9",
    a700: "#aa00ff",
    lighten5: "#f3e5f5",
    lighten4: "#e1bee7",
    lighten3: "#ce93d8",
    lighten2: "#ba68c8",
    lighten1: "#ab47bc",
    darken1: "#8e24aa",
    darken2: "#7b1fa2",
    darken3: "#6a1b9a",
    darken4: "#4a148c",
    accent1: "#ea80fc",
    accent2: "#e040fb",
    accent3: "#d500f9",
    accent4: "#aa00ff"
  },
  deepPurple: {
    base: "#673ab7",
    50: "#ede7f6",
    100: "#d1c4e9",
    200: "#b39ddb",
    300: "#9575cd",
    400: "#7e57c2",
    500: "#673ab7",
    600: "#5e35b1",
    700: "#512da8",
    800: "#4527a0",
    900: "#311b92",
    a100: "#b388ff",
    a200: "#7c4dff",
    a400: "#651fff",
    a700: "#6200ea",
    lighten5: "#ede7f6",
    lighten4: "#d1c4e9",
    lighten3: "#b39ddb",
    lighten2: "#9575cd",
    lighten1: "#7e57c2",
    darken1: "#5e35b1",
    darken2: "#512da8",
    darken3: "#4527a0",
    darken4: "#311b92",
    accent1: "#b388ff",
    accent2: "#7c4dff",
    accent3: "#651fff",
    accent4: "#6200ea"
  },
  indigo: {
    base: "#3f51b5",
    50: "#e8eaf6",
    100: "#c5cae9",
    200: "#9fa8da",
    300: "#7986cb",
    400: "#5c6bc0",
    500: "#3f51b5",
    600: "#3949ab",
    700: "#303f9f",
    800: "#283593",
    900: "#1a237e",
    a100: "#8c9eff",
    a200: "#536dfe",
    a400: "#3d5afe",
    a700: "#304ffe",
    lighten5: "#e8eaf6",
    lighten4: "#c5cae9",
    lighten3: "#9fa8da",
    lighten2: "#7986cb",
    lighten1: "#5c6bc0",
    darken1: "#3949ab",
    darken2: "#303f9f",
    darken3: "#283593",
    darken4: "#1a237e",
    accent1: "#8c9eff",
    accent2: "#536dfe",
    accent3: "#3d5afe",
    accent4: "#304ffe"
  },
  blue: {
    base: "#2196f3",
    50: "#e3f2fd",
    100: "#bbdefb",
    200: "#90caf9",
    300: "#64b5f6",
    400: "#42a5f5",
    500: "#2196f3",
    600: "#1e88e5",
    700: "#1976d2",
    800: "#1565c0",
    900: "#0d47a1",
    a100: "#82b1ff",
    a200: "#448aff",
    a400: "#2979ff",
    a700: "#2962ff",
    lighten5: "#e3f2fd",
    lighten4: "#bbdefb",
    lighten3: "#90caf9",
    lighten2: "#64b5f6",
    lighten1: "#42a5f5",
    darken1: "#1e88e5",
    darken2: "#1976d2",
    darken3: "#1565c0",
    darken4: "#0d47a1",
    accent1: "#82b1ff",
    accent2: "#448aff",
    accent3: "#2979ff",
    accent4: "#2962ff"
  },
  lightBlue: {
    base: "#03a9f4",
    50: "#e1f5fe",
    100: "#b3e5fc",
    200: "#81d4fa",
    300: "#4fc3f7",
    400: "#29b6f6",
    500: "#03a9f4",
    600: "#039be5",
    700: "#0288d1",
    800: "#0277bd",
    900: "#01579b",
    a100: "#80d8ff",
    a200: "#40c4ff",
    a400: "#00b0ff",
    a700: "#0091ea",
    lighten5: "#e1f5fe",
    lighten4: "#b3e5fc",
    lighten3: "#81d4fa",
    lighten2: "#4fc3f7",
    lighten1: "#29b6f6",
    darken1: "#039be5",
    darken2: "#0288d1",
    darken3: "#0277bd",
    darken4: "#01579b",
    accent1: "#80d8ff",
    accent2: "#40c4ff",
    accent3: "#00b0ff",
    accent4: "#0091ea"
  },
  cyan: {
    base: "#00bcd4",
    50: "#e0f7fa",
    100: "#b2ebf2",
    200: "#80deea",
    300: "#4dd0e1",
    400: "#26c6da",
    500: "#00bcd4",
    600: "#00acc1",
    700: "#0097a7",
    800: "#00838f",
    900: "#006064",
    a100: "#84ffff",
    a200: "#18ffff",
    a400: "#00e5ff",
    a700: "#00b8d4",
    lighten5: "#e0f7fa",
    lighten4: "#b2ebf2",
    lighten3: "#80deea",
    lighten2: "#4dd0e1",
    lighten1: "#26c6da",
    darken1: "#00acc1",
    darken2: "#0097a7",
    darken3: "#00838f",
    darken4: "#006064",
    accent1: "#84ffff",
    accent2: "#18ffff",
    accent3: "#00e5ff",
    accent4: "#00b8d4"
  },
  teal: {
    base: "#009688",
    50: "#e0f2f1",
    100: "#b2dfdb",
    200: "#80cbc4",
    300: "#4db6ac",
    400: "#26a69a",
    500: "#009688",
    600: "#00897b",
    700: "#00796b",
    800: "#00695c",
    900: "#004d40",
    a100: "#a7ffeb",
    a200: "#64ffda",
    a400: "#1de9b6",
    a700: "#00bfa5",
    lighten5: "#e0f2f1",
    lighten4: "#b2dfdb",
    lighten3: "#80cbc4",
    lighten2: "#4db6ac",
    lighten1: "#26a69a",
    darken1: "#00897b",
    darken2: "#00796b",
    darken3: "#00695c",
    darken4: "#004d40",
    accent1: "#a7ffeb",
    accent2: "#64ffda",
    accent3: "#1de9b6",
    accent4: "#00bfa5"
  },
  green: {
    base: "#4caf50",
    50: "#e8f5e9",
    100: "#c8e6c9",
    200: "#a5d6a7",
    300: "#81c784",
    400: "#66bb6a",
    500: "#4caf50",
    600: "#43a047",
    700: "#388e3c",
    800: "#2e7d32",
    900: "#1b5e20",
    a100: "#b9f6ca",
    a200: "#69f0ae",
    a400: "#00e676",
    a700: "#00c853",
    lighten5: "#e8f5e9",
    lighten4: "#c8e6c9",
    lighten3: "#a5d6a7",
    lighten2: "#81c784",
    lighten1: "#66bb6a",
    darken1: "#43a047",
    darken2: "#388e3c",
    darken3: "#2e7d32",
    darken4: "#1b5e20",
    accent1: "#b9f6ca",
    accent2: "#69f0ae",
    accent3: "#00e676",
    accent4: "#00c853"
  },
  lightGreen: {
    base: "#8bc34a",
    50: "#f1f8e9",
    100: "#dcedc8",
    200: "#c5e1a5",
    300: "#aed581",
    400: "#9ccc65",
    500: "#8bc34a",
    600: "#7cb342",
    700: "#689f38",
    800: "#558b2f",
    900: "#33691e",
    a100: "#ccff90",
    a200: "#b2ff59",
    a400: "#76ff03",
    a700: "#64dd17",
    lighten5: "#f1f8e9",
    lighten4: "#dcedc8",
    lighten3: "#c5e1a5",
    lighten2: "#aed581",
    lighten1: "#9ccc65",
    darken1: "#7cb342",
    darken2: "#689f38",
    darken3: "#558b2f",
    darken4: "#33691e",
    accent1: "#ccff90",
    accent2: "#b2ff59",
    accent3: "#76ff03",
    accent4: "#64dd17"
  },
  lime: {
    base: "#cddc39",
    50: "#f9fbe7",
    100: "#f0f4c3",
    200: "#e6ee9c",
    300: "#dce775",
    400: "#d4e157",
    500: "#cddc39",
    600: "#c0ca33",
    700: "#afb42b",
    800: "#9e9d24",
    900: "#827717",
    a100: "#f4ff81",
    a200: "#eeff41",
    a400: "#c6ff00",
    a700: "#aeea00",
    lighten5: "#f9fbe7",
    lighten4: "#f0f4c3",
    lighten3: "#e6ee9c",
    lighten2: "#dce775",
    lighten1: "#d4e157",
    darken1: "#c0ca33",
    darken2: "#afb42b",
    darken3: "#9e9d24",
    darken4: "#827717",
    accent1: "#f4ff81",
    accent2: "#eeff41",
    accent3: "#c6ff00",
    accent4: "#aeea00"
  },
  yellow: {
    base: "#ffeb3b",
    50: "#fffde7",
    100: "#fff9c4",
    200: "#fff59d",
    300: "#fff176",
    400: "#ffee58",
    500: "#ffeb3b",
    600: "#fdd835",
    700: "#fbc02d",
    800: "#f9a825",
    900: "#f57f17",
    a100: "#ffff8d",
    a200: "#ffff00",
    a400: "#ffea00",
    a700: "#ffd600",
    lighten5: "#fffde7",
    lighten4: "#fff9c4",
    lighten3: "#fff59d",
    lighten2: "#fff176",
    lighten1: "#ffee58",
    darken1: "#fdd835",
    darken2: "#fbc02d",
    darken3: "#f9a825",
    darken4: "#f57f17",
    accent1: "#ffff8d",
    accent2: "#ffff00",
    accent3: "#ffea00",
    accent4: "#ffd600"
  },
  amber: {
    base: "#ffc107",
    50: "#fff8e1",
    100: "#ffecb3",
    200: "#ffe082",
    300: "#ffd54f",
    400: "#ffca28",
    500: "#ffc107",
    600: "#ffb300",
    700: "#ffa000",
    800: "#ff8f00",
    900: "#ff6f00",
    a100: "#ffe57f",
    a200: "#ffd740",
    a400: "#ffc400",
    a700: "#ffab00",
    lighten5: "#fff8e1",
    lighten4: "#ffecb3",
    lighten3: "#ffe082",
    lighten2: "#ffd54f",
    lighten1: "#ffca28",
    darken1: "#ffb300",
    darken2: "#ffa000",
    darken3: "#ff8f00",
    darken4: "#ff6f00",
    accent1: "#ffe57f",
    accent2: "#ffd740",
    accent3: "#ffc400",
    accent4: "#ffab00"
  },
  orange: {
    base: "#ff9800",
    50: "#fff3e0",
    100: "#ffe0b2",
    200: "#ffcc80",
    300: "#ffb74d",
    400: "#ffa726",
    500: "#ff9800",
    600: "#fb8c00",
    700: "#f57c00",
    800: "#ef6c00",
    900: "#e65100",
    a100: "#ffd180",
    a200: "#ffab40",
    a400: "#ff9100",
    a700: "#ff6d00",
    lighten5: "#fff3e0",
    lighten4: "#ffe0b2",
    lighten3: "#ffcc80",
    lighten2: "#ffb74d",
    lighten1: "#ffa726",
    darken1: "#fb8c00",
    darken2: "#f57c00",
    darken3: "#ef6c00",
    darken4: "#e65100",
    accent1: "#ffd180",
    accent2: "#ffab40",
    accent3: "#ff9100",
    accent4: "#ff6d00"
  },
  deepOrange: {
    base: "#ff5722",
    50: "#fbe9e7",
    100: "#ffccbc",
    200: "#ffab91",
    300: "#ff8a65",
    400: "#ff7043",
    500: "#ff5722",
    600: "#f4511e",
    700: "#e64a19",
    800: "#d84315",
    900: "#bf360c",
    a100: "#ff9e80",
    a200: "#ff6e40",
    a400: "#ff3d00",
    a700: "#dd2c00",
    lighten5: "#fbe9e7",
    lighten4: "#ffccbc",
    lighten3: "#ffab91",
    lighten2: "#ff8a65",
    lighten1: "#ff7043",
    darken1: "#f4511e",
    darken2: "#e64a19",
    darken3: "#d84315",
    darken4: "#bf360c",
    accent1: "#ff9e80",
    accent2: "#ff6e40",
    accent3: "#ff3d00",
    accent4: "#dd2c00"
  },
  brown: {
    base: "#795548",
    50: "#efebe9",
    100: "#d7ccc8",
    200: "#bcaaa4",
    300: "#a1887f",
    400: "#8d6e63",
    500: "#795548",
    600: "#6d4c41",
    700: "#5d4037",
    800: "#4e342e",
    900: "#3e2723",
    lighten5: "#efebe9",
    lighten4: "#d7ccc8",
    lighten3: "#bcaaa4",
    lighten2: "#a1887f",
    lighten1: "#8d6e63",
    darken1: "#6d4c41",
    darken2: "#5d4037",
    darken3: "#4e342e",
    darken4: "#3e2723"
  },
  grey: {
    base: "#9e9e9e",
    50: "#fafafa",
    100: "#f5f5f5",
    200: "#eeeeee",
    300: "#e0e0e0",
    400: "#bdbdbd",
    500: "#9e9e9e",
    600: "#757575",
    700: "#616161",
    800: "#424242",
    900: "#212121",
    lighten5: "#fafafa",
    lighten4: "#f5f5f5",
    lighten3: "#eeeeee",
    lighten2: "#e0e0e0",
    lighten1: "#bdbdbd",
    darken1: "#757575",
    darken2: "#616161",
    darken3: "#424242",
    darken4: "#212121"
  },
  blueGrey: {
    base: "#607d8b",
    50: "#eceff1",
    100: "#cfd8dc",
    200: "#b0bec5",
    300: "#90a4ae",
    400: "#78909c",
    500: "#607d8b",
    600: "#546e7a",
    700: "#455a64",
    800: "#37474f",
    900: "#263238",
    lighten5: "#eceff1",
    lighten4: "#cfd8dc",
    lighten3: "#b0bec5",
    lighten2: "#90a4ae",
    lighten1: "#78909c",
    darken1: "#546e7a",
    darken2: "#455a64",
    darken3: "#37474f",
    darken4: "#263238"
  },
  shades: {
    black: "#000000",
    white: "#ffffff",
    transparent: "transparent"
  }
}, J = {
  deepPurple: "deep-purple",
  lightBlue: "light-blue",
  lightGreen: "light-green",
  deepOrange: "deep-orange",
  blueGrey: "blue-grey"
}, j = {
  lighten5: "lighten-5",
  lighten4: "lighten-4",
  lighten3: "lighten-3",
  lighten2: "lighten-2",
  lighten1: "lighten-1",
  darken1: "darken-1",
  darken2: "darken-2",
  darken3: "darken-3",
  darken4: "darken-4",
  accent1: "accent-1",
  accent2: "accent-2",
  accent3: "accent-3",
  accent4: "accent-4"
};
function Q(e) {
  const t = {};
  for (const [r, f] of Object.entries(e)) {
    if (r === "shades") {
      t.black = f.black, t.white = f.white;
      continue;
    }
    const c = J[r] ?? r;
    t[c] = f.base;
    for (const [i, h] of Object.entries(f)) {
      const a = j[i];
      a && (t[`${c}-${a}`] = h);
    }
  }
  return t;
}
function z() {
  if (typeof document > "u" || document.getElementById("prism-color-tokens"))
    return;
  const e = [];
  for (const [r, f] of Object.entries(w)) {
    if (r === "shades") {
      e.push("--prism-color-black: #000000"), e.push("--prism-color-white: #ffffff"), e.push("--prism-color-transparent: transparent"), e.push("--prism-color-scrim: rgba(0, 0, 0, 0.3)");
      continue;
    }
    const c = J[r] ?? r;
    e.push(`--prism-color-${c}-base: ${f.base}`);
    for (const [i, h] of Object.entries(f)) {
      const a = j[i];
      a && e.push(`--prism-color-${c}-${a}: ${h}`);
    }
  }
  const t = document.createElement("style");
  t.id = "prism-color-tokens", t.textContent = `:root {
  ${e.join(`;
  `)}
}`, document.head.insertBefore(t, document.head.firstChild);
}
const Et = {
  dark: !1,
  colors: {
    // Semantic brand colors
    primary: "#277ffd",
    secondary: w.orange.base,
    accent: w.lime.base,
    error: w.red.base,
    warning: w.amber.darken2,
    success: w.green.base,
    info: "#277ffd",
    // Full Material Design palette
    ...Q(w)
  }
};
const m = (e, t) => {
  const r = e.__vccOpts || e;
  for (const [f, c] of t)
    r[f] = c;
  return r;
}, G = {
  name: "PrismButton",
  props: {
    variant: {
      type: String,
      default: "primary",
      validator: (e) => [
        "primary",
        "secondary",
        "ghost",
        "danger",
        "gradient-primary",
        "gradient-sunset",
        "gradient-ocean",
        "gradient-forest",
        "gradient-aurora",
        "gradient-fire"
      ].includes(e)
    },
    size: {
      type: String,
      default: "md",
      validator: (e) => ["sm", "md", "lg"].includes(e)
    },
    type: {
      type: String,
      default: "button"
    },
    disabled: {
      type: Boolean,
      default: !1
    }
  },
  methods: {
    onClick(e) {
      this.$emit("click", e);
    }
  }
}, V = ["type", "disabled"];
function q(e, t, r, f, c, i) {
  return n(), s("button", {
    type: r.type,
    class: u(["prism-button", `prism-button--${r.variant}`, `prism-button--${r.size}`, { "is-disabled": r.disabled }]),
    disabled: r.disabled,
    onClick: t[0] || (t[0] = (...h) => i.onClick && i.onClick(...h))
  }, [
    y(e.$slots, "default", {}, void 0, !0)
  ], 10, V);
}
const I = /* @__PURE__ */ m(G, [["render", q], ["__scopeId", "data-v-bf515133"]]);
I.install = function(t) {
  t.component(I.name, I);
};
const Y = {
  name: "PrismHeroBanner",
  props: {
    image: {
      type: String,
      default: ""
    },
    title: {
      type: String,
      default: ""
    },
    body: {
      type: String,
      default: ""
    },
    variant: {
      type: String,
      default: "primary"
    }
  }
}, Z = { class: "hero__content" }, $ = { class: "hero__title" }, ee = { class: "hero__description" };
function te(e, t, r, f, c, i) {
  return n(), s("section", {
    class: u(["hero__container", `hero__container--${r.variant} ${r.image ? "hero__container--image" : ""}`]),
    style: W(r.image ? {
      backgroundImage: `url(${r.image})`,
      backgroundSize: "cover",
      backgroundPosition: "center"
    } : {})
  }, [
    l("div", Z, [
      l("h2", $, o(r.title), 1),
      l("div", ee, [
        l("p", null, o(r.body), 1),
        y(e.$slots, "default", {}, void 0, !0)
      ])
    ])
  ], 6);
}
const S = /* @__PURE__ */ m(Y, [["render", te], ["__scopeId", "data-v-e0d066b3"]]);
S.install = function(t) {
  t.component(S.name, S);
};
const ae = "data:image/png;base64,";
function F() {
  try {
    const e = JSON.parse(localStorage.getItem("user_info"));
    return !!e && !e.isAnonymous;
  } catch {
    return !1;
  }
}
const ne = { marketing: "", portal: "/portal" }, re = [
  { title: "About us", children: [
    { title: "About PRISM", route: "/about-us/about-prism", id: "about-prism", site: "marketing" },
    { title: "Team", route: "/about-us/team", id: "team", site: "marketing" }
  ] },
  { title: "Resources", children: [
    { title: "Publications", route: "/publications", id: "publications", site: "marketing" },
    { title: "FAQ", route: "/faq", id: "faq", site: "marketing" },
    { title: "Webinars", route: "/webinars", id: "webinars", site: "marketing" }
  ] },
  { title: "Consortium Screens", children: [
    { title: "Cell line collection", route: "/consortium-screens/cell-line-collection", id: "cell-line-collection", site: "marketing" },
    { title: "Assays", route: "/consortium-screens/assays", id: "assays", site: "marketing" },
    { title: "Data Analysis", route: "/consortium-screens/data-analysis", id: "data-analysis", site: "marketing" },
    { title: "Deliverables", route: "/consortium-screens/deliverables", id: "deliverables", site: "marketing" },
    { title: "Collaborating", route: "/consortium-screens/collaborating", id: "collaborating", site: "marketing" },
    { title: "Submission Hub", route: "/submission-hub", id: "submission-hub", matchPrefix: !0, site: "marketing" }
  ] },
  { title: "Contact us", route: "/contact-us", id: "contact-us", site: "marketing" },
  { title: "Portal", route: "/portal", id: "portal", matchPrefix: !0, site: "portal", excludeRoutes: ["/portal/log-in"] }
], se = { route: "/", site: "marketing" }, ie = { route: "/portal/log-in", site: "portal" };
function P(e, t, r) {
  if (e.children)
    return {
      ...e,
      children: e.children.map((f) => P(f, t, r))
    };
  if (!e.route)
    return e;
  if (!e.site || e.site === t) {
    const f = ne[t] || "", c = f && e.route.startsWith(f), i = c ? e.route.slice(f.length) || "/" : e.route;
    return {
      ...e,
      route: i,
      external: !1,
      // When matchPrefix and the base was stripped, preserve the original route so
      // active-state checking uses the real browser path prefix (e.g. '/portal'),
      // not '/' which would match every URL.
      ...e.matchPrefix && c && { activePrefix: e.route }
    };
  }
  return { ...e, route: `${r}${e.route}`, external: !0 };
}
const le = {
  name: "PrismAppBar",
  props: {
    site: {
      type: String,
      default: "marketing",
      validator: (e) => ["marketing", "portal"].includes(e)
    },
    baseUrl: {
      type: String,
      default: ""
    }
  },
  data() {
    return {
      prismLogo: ae,
      drawerOpen: !1,
      openGroups: [],
      isLoggedIn: F()
    };
  },
  mounted() {
    window.addEventListener("storage", this._onAuthChange);
  },
  beforeDestroy() {
    window.removeEventListener("storage", this._onAuthChange);
  },
  computed: {
    resolvedLogo() {
      return P(se, this.site, this.baseUrl);
    },
    resolvedItems() {
      return re.map((e) => P(e, this.site, this.baseUrl));
    },
    resolvedLoginItem() {
      return P(ie, this.site, this.baseUrl);
    }
  },
  methods: {
    _onAuthChange(e) {
      (e.key === "user_info" || e.key === null) && (this.isLoggedIn = F());
    },
    _currentPath() {
      var e;
      return ((e = this.$route) == null ? void 0 : e.path) ?? window.location.pathname;
    },
    _matchPath(e) {
      if (!e)
        return !1;
      try {
        return new URL(e, window.location.href).pathname === this._currentPath();
      } catch {
        return e === this._currentPath();
      }
    },
    isParentActive(e) {
      return !e.children || !e.children.length ? !1 : e.children.some((t) => this.isItemActive(t));
    },
    isItemActive(e) {
      if (e.children && e.children.length)
        return this.isParentActive(e);
      if (e.matchPrefix) {
        const t = e.activePrefix || e.route;
        try {
          const r = new URL(t, window.location.href).pathname, f = window.location.pathname.startsWith(r);
          return f && e.excludeRoutes ? !e.excludeRoutes.includes(window.location.pathname) : f;
        } catch {
          return window.location.pathname.startsWith(t);
        }
      }
      return this._matchPath(e.route);
    },
    navigateTo(e, t) {
      t !== void 0 && (this.openGroups = [t]), this.drawerOpen = !1, this.$router.push(e).catch(() => {
      });
    },
    selectChild(e) {
      this.openGroups = [e], this.drawerOpen = !1;
    },
    toggleGroup(e) {
      this.openGroups.includes(e) ? this.openGroups = this.openGroups.filter((t) => t !== e) : this.openGroups.push(e);
    }
  }
}, ce = { class: "prism-app-bar-root" }, de = { class: "prism-app-bar" }, oe = { class: "prism-app-bar__inner" }, fe = ["src"], ue = ["aria-expanded"], pe = {
  class: "prism-app-bar__desktop-nav",
  "aria-label": "Main navigation"
}, he = ["href"], be = {
  key: 3,
  class: "prism-app-bar__dropdown"
}, ve = ["href"], ge = { class: "prism-app-bar__cta" }, me = {
  key: 0,
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24",
  fill: "currentColor",
  width: "16",
  height: "16",
  "aria-hidden": "true"
}, _e = {
  key: 0,
  class: "prism-nav-drawer",
  "aria-label": "Mobile navigation"
}, Ae = ["href"], ke = ["href", "onClick"], ye = ["onClick"], we = {
  key: 0,
  class: "prism-nav-drawer__children"
}, Te = ["href", "onClick"], Ie = ["href", "onClick"], Se = { class: "prism-nav-drawer__actions" }, Pe = {
  key: 0,
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24",
  fill: "currentColor",
  width: "16",
  height: "16",
  "aria-hidden": "true"
};
function xe(e, t, r, f, c, i) {
  const h = D("router-link");
  return n(), s("div", ce, [
    l("header", de, [
      l("div", oe, [
        (n(), A(T(i.resolvedLogo.external ? "a" : "router-link"), M(i.resolvedLogo.external ? { href: i.resolvedLogo.route } : { to: i.resolvedLogo.route }, { class: "prism-app-bar__logo-link" }), {
          default: g(() => [
            l("img", {
              src: c.prismLogo,
              alt: "PRISM",
              class: "prism-app-bar__logo-img"
            }, null, 8, fe)
          ]),
          _: 1
        }, 16)),
        l("button", {
          class: "prism-app-bar__hamburger",
          "aria-expanded": String(c.drawerOpen),
          "aria-label": "Toggle navigation",
          onClick: t[0] || (t[0] = (a) => c.drawerOpen = !c.drawerOpen)
        }, [...t[4] || (t[4] = [
          l("span", null, null, -1),
          l("span", null, null, -1),
          l("span", null, null, -1)
        ])], 8, ue),
        l("nav", pe, [
          (n(!0), s(v, null, k(i.resolvedItems, (a, b) => (n(), s("div", {
            key: b,
            class: u(["prism-app-bar__nav-item", { "prism-app-bar__nav-item--has-children": a.children && a.children.length }])
          }, [
            a.children && a.children.length ? (n(), s("button", {
              key: 0,
              type: "button",
              class: u(["prism-app-bar__nav-link", { "is-active": i.isParentActive(a) }])
            }, [
              _(o(a.title) + " ", 1),
              t[5] || (t[5] = l("svg", {
                class: "prism-app-bar__chevron",
                xmlns: "http://www.w3.org/2000/svg",
                viewBox: "0 0 20 20",
                fill: "currentColor",
                width: "16",
                height: "16",
                "aria-hidden": "true"
              }, [
                l("path", {
                  "fill-rule": "evenodd",
                  d: "M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z",
                  "clip-rule": "evenodd"
                })
              ], -1))
            ], 2)) : a.external ? (n(), s("a", {
              key: 1,
              href: a.route,
              class: u(["prism-app-bar__nav-link", { "is-active": i.isItemActive(a) }])
            }, o(a.title), 11, he)) : (n(), A(h, {
              key: 2,
              to: a.route,
              class: u(["prism-app-bar__nav-link", { "is-active": i.isItemActive(a) }])
            }, {
              default: g(() => [
                _(o(a.title), 1)
              ]),
              _: 2
            }, 1032, ["to", "class"])),
            a.children && a.children.length ? (n(), s("div", be, [
              (n(!0), s(v, null, k(a.children, (d) => (n(), s(v, null, [
                d.external ? (n(), s("a", {
                  key: `ext-${d.id}`,
                  href: d.route,
                  class: u(["prism-app-bar__dropdown-link", { "is-active": i.isItemActive(d) }])
                }, o(d.title), 11, ve)) : (n(), A(h, {
                  key: d.id,
                  to: d.route,
                  class: u(["prism-app-bar__dropdown-link", { "is-active": i.isItemActive(d) }])
                }, {
                  default: g(() => [
                    _(o(d.title), 1)
                  ]),
                  _: 2
                }, 1032, ["to", "class"]))
              ], 64))), 256))
            ])) : p("", !0)
          ], 2))), 128)),
          l("div", ge, [
            (n(), A(T(i.resolvedLoginItem.external ? "a" : "router-link"), M(i.resolvedLoginItem.external ? { href: i.resolvedLoginItem.route } : { to: i.resolvedLoginItem.route }, { class: "prism-app-bar__login-btn" }), {
              default: g(() => [
                c.isLoggedIn ? (n(), s("svg", me, [...t[6] || (t[6] = [
                  l("path", { d: "M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z" }, null, -1)
                ])])) : p("", !0),
                _(" " + o(c.isLoggedIn ? "" : "Log In"), 1)
              ]),
              _: 1
            }, 16))
          ])
        ])
      ])
    ]),
    c.drawerOpen ? (n(), s("div", {
      key: 0,
      class: "prism-nav-drawer__backdrop",
      "aria-hidden": "true",
      onClick: t[1] || (t[1] = (a) => c.drawerOpen = !1)
    })) : p("", !0),
    R(X, { name: "prism-drawer-slide" }, {
      default: g(() => [
        c.drawerOpen ? (n(), s("nav", _e, [
          (n(!0), s(v, null, k(i.resolvedItems, (a, b) => (n(), s("div", {
            key: b,
            class: "prism-nav-drawer__item"
          }, [
            !a.children || a.children.length === 0 ? (n(), s(v, { key: 0 }, [
              a.external ? (n(), s("a", {
                key: `ext-${a.id}`,
                href: a.route,
                class: u(["prism-nav-drawer__link", { "is-active": e.$route && e.$route.path === a.route }]),
                onClick: t[2] || (t[2] = (d) => c.drawerOpen = !1)
              }, o(a.title), 11, Ae)) : (n(), s("a", {
                key: a.id,
                href: a.route,
                class: u(["prism-nav-drawer__link", { "is-active": e.$route && e.$route.path === a.route }]),
                onClick: E((d) => i.navigateTo(a.route), ["prevent"])
              }, o(a.title), 11, ke))
            ], 64)) : (n(), s(v, { key: 1 }, [
              l("button", {
                class: u([
                  "prism-nav-drawer__group-btn",
                  { "is-active": i.isParentActive(a), "is-open": c.openGroups.includes(b) }
                ]),
                onClick: (d) => i.toggleGroup(b)
              }, [
                _(o(a.title) + " ", 1),
                t[7] || (t[7] = l("svg", {
                  class: "prism-nav-drawer__chevron",
                  xmlns: "http://www.w3.org/2000/svg",
                  viewBox: "0 0 20 20",
                  fill: "currentColor",
                  width: "16",
                  height: "16",
                  "aria-hidden": "true"
                }, [
                  l("path", {
                    "fill-rule": "evenodd",
                    d: "M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z",
                    "clip-rule": "evenodd"
                  })
                ], -1))
              ], 10, ye),
              c.openGroups.includes(b) ? (n(), s("div", we, [
                (n(!0), s(v, null, k(a.children, (d) => (n(), s(v, null, [
                  d.external ? (n(), s("a", {
                    key: `ext-${d.id}`,
                    href: d.route,
                    class: u(["prism-nav-drawer__child-link", { "is-active": i.isItemActive(d) }]),
                    onClick: (U) => i.selectChild(b)
                  }, o(d.title), 11, Te)) : (n(), s("a", {
                    key: d.id,
                    href: d.route,
                    class: u(["prism-nav-drawer__child-link", { "is-active": i.isItemActive(d) }]),
                    onClick: E((U) => i.navigateTo(d.route, b), ["prevent"])
                  }, o(d.title), 11, Ie))
                ], 64))), 256))
              ])) : p("", !0)
            ], 64))
          ]))), 128)),
          l("div", Se, [
            (n(), A(T(i.resolvedLoginItem.external ? "a" : "router-link"), M(i.resolvedLoginItem.external ? { href: i.resolvedLoginItem.route } : { to: i.resolvedLoginItem.route }, {
              class: "prism-app-bar__login-btn",
              onClick: t[3] || (t[3] = (a) => c.drawerOpen = !1)
            }), {
              default: g(() => [
                c.isLoggedIn ? (n(), s("svg", Pe, [...t[8] || (t[8] = [
                  l("path", { d: "M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z" }, null, -1)
                ])])) : p("", !0),
                _(" " + o(c.isLoggedIn ? "Account" : "Log In"), 1)
              ]),
              _: 1
            }, 16))
          ])
        ])) : p("", !0)
      ]),
      _: 1
    })
  ]);
}
const x = /* @__PURE__ */ m(le, [["render", xe], ["__scopeId", "data-v-758427f8"]]);
x.install = function(t) {
  t.component(x.name, x);
};
const Ce = {
  name: "PrismAlertBanner",
  props: {
    variant: {
      type: String,
      default: "info",
      validator: (e) => ["info", "success", "warning", "error", "neutral"].includes(e)
    },
    title: {
      type: String,
      default: ""
    },
    message: {
      type: String,
      default: ""
    },
    dismissible: {
      type: Boolean,
      default: !1
    },
    bordered: {
      type: Boolean,
      default: !1
    }
  },
  data() {
    return { visible: !0 };
  },
  methods: {
    dismiss() {
      this.visible = !1, this.$emit("dismiss");
    }
  }
}, Oe = {
  class: "prism-alert__icon",
  "aria-hidden": "true"
}, Be = {
  key: 0,
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 20 20",
  fill: "currentColor",
  width: "20",
  height: "20"
}, Ne = {
  key: 1,
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 20 20",
  fill: "currentColor",
  width: "20",
  height: "20"
}, Le = {
  key: 2,
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 20 20",
  fill: "currentColor",
  width: "20",
  height: "20"
}, He = {
  key: 3,
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 20 20",
  fill: "currentColor",
  width: "20",
  height: "20"
}, Ke = { class: "prism-alert__body" }, Me = {
  key: 0,
  class: "prism-alert__title"
}, Re = { class: "prism-alert__message" };
function Xe(e, t, r, f, c, i) {
  return c.visible ? (n(), s("div", {
    key: 0,
    class: u(["prism-alert", `prism-alert--${r.variant}`, { "prism-alert--bordered": r.bordered }]),
    role: "alert"
  }, [
    l("span", Oe, [
      r.variant === "success" ? (n(), s("svg", Be, [...t[1] || (t[1] = [
        l("path", {
          "fill-rule": "evenodd",
          d: "M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z",
          "clip-rule": "evenodd"
        }, null, -1)
      ])])) : r.variant === "warning" ? (n(), s("svg", Ne, [...t[2] || (t[2] = [
        l("path", {
          "fill-rule": "evenodd",
          d: "M8.485 2.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.347 0-2.189-1.458-1.515-2.625L8.485 2.495zM10 5a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 5zm0 9a1 1 0 100-2 1 1 0 000 2z",
          "clip-rule": "evenodd"
        }, null, -1)
      ])])) : r.variant === "error" ? (n(), s("svg", Le, [...t[3] || (t[3] = [
        l("path", {
          "fill-rule": "evenodd",
          d: "M10 18a8 8 0 100-16 8 8 0 000 16zM8.28 7.22a.75.75 0 00-1.06 1.06L8.94 10l-1.72 1.72a.75.75 0 101.06 1.06L10 11.06l1.72 1.72a.75.75 0 101.06-1.06L11.06 10l1.72-1.72a.75.75 0 00-1.06-1.06L10 8.94 8.28 7.22z",
          "clip-rule": "evenodd"
        }, null, -1)
      ])])) : (n(), s("svg", He, [...t[4] || (t[4] = [
        l("path", {
          "fill-rule": "evenodd",
          d: "M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.253a.25.25 0 01.244.304l-.459 2.066A1.75 1.75 0 0010.747 15H11a.75.75 0 000-1.5h-.253a.25.25 0 01-.244-.304l.459-2.066A1.75 1.75 0 009.253 9H9z",
          "clip-rule": "evenodd"
        }, null, -1)
      ])]))
    ]),
    l("div", Ke, [
      r.title ? (n(), s("p", Me, o(r.title), 1)) : p("", !0),
      l("div", Re, [
        y(e.$slots, "default", {}, () => [
          _(o(r.message), 1)
        ], !0)
      ])
    ]),
    r.dismissible ? (n(), s("button", {
      key: 0,
      class: "prism-alert__close",
      "aria-label": "Dismiss",
      onClick: t[0] || (t[0] = (...h) => i.dismiss && i.dismiss(...h))
    }, [...t[5] || (t[5] = [
      l("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 20 20",
        fill: "currentColor",
        width: "16",
        height: "16",
        "aria-hidden": "true"
      }, [
        l("path", { d: "M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" })
      ], -1)
    ])])) : p("", !0)
  ], 2)) : p("", !0);
}
const C = /* @__PURE__ */ m(Ce, [["render", Xe], ["__scopeId", "data-v-89ca1dc5"]]);
C.install = function(t) {
  t.component(C.name, C);
};
const We = {
  name: "PrismSplitSection",
  props: {
    image: {
      type: String,
      default: ""
    },
    imageAlt: {
      type: String,
      default: ""
    },
    eyebrow: {
      type: String,
      default: ""
    },
    title: {
      type: String,
      default: ""
    },
    body: {
      type: String,
      default: ""
    },
    reverse: {
      type: Boolean,
      default: !1
    }
  }
}, De = { class: "prism-split__image-col" }, Ee = ["src", "alt"], Fe = { class: "prism-split__content-col" }, Je = {
  key: 0,
  class: "prism-split__eyebrow"
}, je = {
  key: 1,
  class: "prism-split__title"
}, Ue = {
  key: 2,
  class: "prism-split__body"
}, Qe = {
  key: 3,
  class: "prism-split__actions"
};
function ze(e, t, r, f, c, i) {
  return n(), s("section", {
    class: u(["prism-split", { "prism-split--reverse": r.reverse }])
  }, [
    l("div", De, [
      r.image ? (n(), s("img", {
        key: 0,
        src: r.image,
        alt: r.imageAlt,
        class: "prism-split__image"
      }, null, 8, Ee)) : p("", !0),
      y(e.$slots, "image", {}, void 0, !0)
    ]),
    l("div", Fe, [
      r.eyebrow ? (n(), s("p", Je, o(r.eyebrow), 1)) : p("", !0),
      r.title ? (n(), s("h2", je, o(r.title), 1)) : p("", !0),
      r.body || e.$slots.default ? (n(), s("div", Ue, [
        y(e.$slots, "default", {}, () => [
          _(o(r.body), 1)
        ], !0)
      ])) : p("", !0),
      e.$slots.actions ? (n(), s("div", Qe, [
        y(e.$slots, "actions", {}, void 0, !0)
      ])) : p("", !0)
    ])
  ], 2);
}
const O = /* @__PURE__ */ m(We, [["render", ze], ["__scopeId", "data-v-844b1b37"]]);
O.install = function(t) {
  t.component(O.name, O);
};
const Ge = {
  name: "PrismTimeline",
  props: {
    steps: {
      type: Array,
      required: !0
    },
    activeStep: {
      type: Number,
      default: -1
    },
    variant: {
      type: String,
      default: "vertical",
      validator: (e) => ["vertical", "horizontal"].includes(e)
    }
  }
}, Ve = { class: "prism-timeline__marker" }, qe = {
  key: 0,
  class: "prism-timeline__check",
  "aria-hidden": "true"
}, Ye = {
  key: 1,
  class: "prism-timeline__number"
}, Ze = { class: "prism-timeline__content" }, $e = { class: "prism-timeline__title" }, et = {
  key: 0,
  class: "prism-timeline__caption"
};
function tt(e, t, r, f, c, i) {
  return n(), s("ol", {
    class: u(["prism-timeline", `prism-timeline--${r.variant}`])
  }, [
    (n(!0), s(v, null, k(r.steps, (h, a) => (n(), s("li", {
      key: a,
      class: u(["prism-timeline__step", { "prism-timeline__step--active": a === r.activeStep, "prism-timeline__step--completed": a < r.activeStep }])
    }, [
      t[1] || (t[1] = l("div", {
        class: "prism-timeline__connector",
        "aria-hidden": "true"
      }, null, -1)),
      l("div", Ve, [
        a < r.activeStep ? (n(), s("span", qe, [...t[0] || (t[0] = [
          l("svg", {
            xmlns: "http://www.w3.org/2000/svg",
            viewBox: "0 0 20 20",
            fill: "currentColor",
            width: "14",
            height: "14"
          }, [
            l("path", {
              "fill-rule": "evenodd",
              d: "M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z",
              "clip-rule": "evenodd"
            })
          ], -1)
        ])])) : (n(), s("span", Ye, o(a + 1), 1))
      ]),
      l("div", Ze, [
        l("p", $e, o(h.title), 1),
        h.caption ? (n(), s("p", et, o(h.caption), 1)) : p("", !0)
      ])
    ], 2))), 128))
  ], 2);
}
const B = /* @__PURE__ */ m(Ge, [["render", tt], ["__scopeId", "data-v-fb40f417"]]);
B.install = function(t) {
  t.component(B.name, B);
};
const at = {
  name: "PrismAppNavDrawer",
  props: {
    items: {
      type: Array,
      default: () => []
    },
    top: {
      type: String,
      default: "64px"
    }
  },
  data() {
    return {
      isExpanded: !1
    };
  },
  methods: {
    isActive(e) {
      var r;
      return (((r = this.$route) == null ? void 0 : r.path) ?? "") === e.route;
    }
  }
}, nt = { class: "prism-app-nav-drawer__list" }, rt = ["href", "title"], st = ["innerHTML"], it = { class: "prism-app-nav-drawer__label" }, lt = ["innerHTML"], ct = { class: "prism-app-nav-drawer__label" };
function dt(e, t, r, f, c, i) {
  const h = D("router-link");
  return n(), s("nav", {
    class: u(["prism-app-nav-drawer", { "is-expanded": c.isExpanded }]),
    style: W({ top: r.top }),
    onMouseenter: t[0] || (t[0] = (a) => c.isExpanded = !0),
    onMouseleave: t[1] || (t[1] = (a) => c.isExpanded = !1),
    "aria-label": "Side navigation"
  }, [
    l("ul", nt, [
      (n(!0), s(v, null, k(r.items, (a, b) => (n(), s("li", {
        key: b,
        class: "prism-app-nav-drawer__item"
      }, [
        a.external ? (n(), s("a", {
          key: 0,
          href: a.route,
          title: a.title,
          class: u(["prism-app-nav-drawer__link", { "is-active": i.isActive(a) }])
        }, [
          l("span", {
            class: "prism-app-nav-drawer__icon",
            innerHTML: a.icon,
            "aria-hidden": "true"
          }, null, 8, st),
          l("span", it, o(a.title), 1)
        ], 10, rt)) : (n(), A(h, {
          key: 1,
          to: a.route,
          title: a.title,
          class: u(["prism-app-nav-drawer__link", { "is-active": e.$route && e.$route.path === a.route }])
        }, {
          default: g(() => [
            l("span", {
              class: "prism-app-nav-drawer__icon",
              innerHTML: a.icon,
              "aria-hidden": "true"
            }, null, 8, lt),
            l("span", ct, o(a.title), 1)
          ]),
          _: 2
        }, 1032, ["to", "title", "class"]))
      ]))), 128))
    ])
  ], 38);
}
const N = /* @__PURE__ */ m(at, [["render", dt], ["__scopeId", "data-v-0dfe6ab6"]]);
N.install = function(t) {
  t.component(N.name, N);
};
const ot = {
  name: "PrismPageTitle"
}, ft = {
  class: "prism-text-display-large",
  style: { margin: "1rem 0 2rem 0" }
};
function ut(e, t, r, f, c, i) {
  return n(), s("h1", ft, [
    y(e.$slots, "default")
  ]);
}
const L = /* @__PURE__ */ m(ot, [["render", ut]]);
L.install = function(t) {
  t.component(L.name, L);
};
const pt = {
  name: "PrismSectionTitle",
  props: {
    tag: {
      type: String,
      default: "h2",
      validator: (e) => ["h1", "h2", "h3", "h4", "h5", "h6"].includes(e)
    }
  }
};
function ht(e, t, r, f, c, i) {
  return n(), A(T(r.tag), {
    class: "prism-text-display-medium",
    style: { margin: "1rem 0 2rem 0" }
  }, {
    default: g(() => [
      y(e.$slots, "default")
    ]),
    _: 3
  });
}
const H = /* @__PURE__ */ m(pt, [["render", ht]]);
H.install = function(t) {
  t.component(H.name, H);
};
const bt = {
  name: "PrismSubSectionTitle"
}, vt = { class: "prism-text-h3" };
function gt(e, t, r, f, c, i) {
  return n(), s("h3", vt, [
    y(e.$slots, "default")
  ]);
}
const K = /* @__PURE__ */ m(bt, [["render", gt]]);
K.install = function(t) {
  t.component(K.name, K);
};
const mt = {
  name: "PrismSubNav",
  props: {
    /**
     * Navigation items. Each item:
     *   { id, title, route, icon?, external?, matchPrefix?, children?: [...same shape] }
     */
    items: {
      type: Array,
      default: () => []
    },
    /**
     * CSS value for the desktop sidebar's top offset — set to match your AppBar height.
     */
    top: {
      type: String,
      default: "64px"
    },
    /**
     * CSS value for the desktop sidebar's left offset — set to match your AppNavDrawer width.
     */
    left: {
      type: String,
      default: "0px"
    }
  },
  data() {
    return {
      openGroups: [],
      mobileExpanded: !1,
      activeTabIndex: null
    };
  },
  computed: {
    activeMobileChildren() {
      if (this.activeTabIndex === null)
        return [];
      const e = this.items[this.activeTabIndex];
      return e && e.children ? e.children : [];
    }
  },
  watch: {
    $route() {
      this.syncState();
    }
  },
  mounted() {
    this.syncState();
  },
  methods: {
    hasChildren(e) {
      return Array.isArray(e.children) && e.children.length > 0;
    },
    currentPath() {
      var e;
      return ((e = this.$route) == null ? void 0 : e.path) ?? window.location.pathname;
    },
    isItemActive(e) {
      const t = this.currentPath();
      return e.matchPrefix ? t.startsWith(e.route) : t === e.route;
    },
    isParentActive(e) {
      return this.hasChildren(e) ? e.children.some((t) => this.isItemActive(t)) : !1;
    },
    toggleGroup(e) {
      this.openGroups.includes(e) ? this.openGroups = this.openGroups.filter((t) => t !== e) : this.openGroups = [...this.openGroups, e];
    },
    onTabClick(e, t) {
      this.hasChildren(e) ? this.activeTabIndex === t && this.mobileExpanded ? this.mobileExpanded = !1 : (this.activeTabIndex = t, this.mobileExpanded = !0) : (this.activeTabIndex = t, this.mobileExpanded = !1, e.external ? window.location.href = e.route : this.$router.push(e.route).catch(() => {
      }));
    },
    syncState() {
      this.items.forEach((t, r) => {
        this.hasChildren(t) && this.isParentActive(t) && (this.openGroups.includes(r) || this.openGroups.push(r));
      });
      const e = this.items.findIndex(
        (t) => this.isItemActive(t) || this.isParentActive(t)
      );
      e >= 0 && (this.activeTabIndex = e);
    }
  }
}, _t = {
  class: "prism-sub-nav__sidebar",
  "aria-label": "Section navigation"
}, At = ["onClick"], kt = ["innerHTML"], yt = { class: "prism-sub-nav__sidebar-label" }, wt = {
  key: 0,
  class: "prism-sub-nav__sidebar-children"
}, Tt = ["href"], It = ["href"], St = ["innerHTML"], Pt = { class: "prism-sub-nav__sidebar-label" }, xt = ["innerHTML"], Ct = { class: "prism-sub-nav__sidebar-label" }, Ot = { class: "prism-sub-nav__bottom" }, Bt = {
  key: 0,
  class: "prism-sub-nav__panel"
}, Nt = ["href"], Lt = {
  class: "prism-sub-nav__tabs",
  "aria-label": "Section navigation"
}, Ht = ["onClick"], Kt = ["innerHTML"], Mt = { class: "prism-sub-nav__tab-label" };
function Rt(e, t, r, f, c, i) {
  const h = D("router-link");
  return n(), s("div", {
    class: "prism-sub-nav",
    style: W({ "--prism-sub-nav-top": r.top, "--prism-sub-nav-left": r.left })
  }, [
    l("nav", _t, [
      (n(!0), s(v, null, k(r.items, (a, b) => (n(), s("div", {
        key: a.id || b,
        class: "prism-sub-nav__sidebar-item"
      }, [
        i.hasChildren(a) ? (n(), s(v, { key: 0 }, [
          l("button", {
            class: u(["prism-sub-nav__sidebar-group", { "is-active": i.isParentActive(a), "is-open": c.openGroups.includes(b) }]),
            onClick: (d) => i.toggleGroup(b)
          }, [
            a.icon ? (n(), s("span", {
              key: 0,
              class: "prism-sub-nav__sidebar-icon",
              innerHTML: a.icon,
              "aria-hidden": "true"
            }, null, 8, kt)) : p("", !0),
            l("span", yt, o(a.title), 1),
            t[2] || (t[2] = l("svg", {
              class: "prism-sub-nav__chevron",
              xmlns: "http://www.w3.org/2000/svg",
              viewBox: "0 0 20 20",
              fill: "currentColor",
              width: "14",
              height: "14",
              "aria-hidden": "true"
            }, [
              l("path", {
                "fill-rule": "evenodd",
                d: "M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z",
                "clip-rule": "evenodd"
              })
            ], -1))
          ], 10, At),
          R(X, { name: "prism-sub-nav-expand" }, {
            default: g(() => [
              c.openGroups.includes(b) ? (n(), s("div", wt, [
                (n(!0), s(v, null, k(a.children, (d) => (n(), s(v, null, [
                  d.external ? (n(), s("a", {
                    key: `ext-${d.id || d.route}`,
                    href: d.route,
                    class: u(["prism-sub-nav__sidebar-child", { "is-active": i.isItemActive(d) }])
                  }, o(d.title), 11, Tt)) : (n(), A(h, {
                    key: d.id || d.route,
                    to: d.route,
                    class: u(["prism-sub-nav__sidebar-child", { "is-active": i.isItemActive(d) }])
                  }, {
                    default: g(() => [
                      _(o(d.title), 1)
                    ]),
                    _: 2
                  }, 1032, ["to", "class"]))
                ], 64))), 256))
              ])) : p("", !0)
            ]),
            _: 2
          }, 1024)
        ], 64)) : (n(), s(v, { key: 1 }, [
          a.external ? (n(), s("a", {
            key: 0,
            href: a.route,
            class: u(["prism-sub-nav__sidebar-link", { "is-active": i.isItemActive(a) }])
          }, [
            a.icon ? (n(), s("span", {
              key: 0,
              class: "prism-sub-nav__sidebar-icon",
              innerHTML: a.icon,
              "aria-hidden": "true"
            }, null, 8, St)) : p("", !0),
            l("span", Pt, o(a.title), 1)
          ], 10, It)) : (n(), A(h, {
            key: 1,
            to: a.route,
            class: u(["prism-sub-nav__sidebar-link", { "is-active": i.isItemActive(a) }])
          }, {
            default: g(() => [
              a.icon ? (n(), s("span", {
                key: 0,
                class: "prism-sub-nav__sidebar-icon",
                innerHTML: a.icon,
                "aria-hidden": "true"
              }, null, 8, xt)) : p("", !0),
              l("span", Ct, o(a.title), 1)
            ]),
            _: 2
          }, 1032, ["to", "class"]))
        ], 64))
      ]))), 128))
    ]),
    l("div", Ot, [
      R(X, { name: "prism-sub-nav-slide" }, {
        default: g(() => [
          c.mobileExpanded && i.activeMobileChildren.length ? (n(), s("div", Bt, [
            (n(!0), s(v, null, k(i.activeMobileChildren, (a) => (n(), s(v, null, [
              a.external ? (n(), s("a", {
                key: `ext-${a.id || a.route}`,
                href: a.route,
                class: u(["prism-sub-nav__panel-link", { "is-active": i.isItemActive(a) }]),
                onClick: t[0] || (t[0] = (b) => c.mobileExpanded = !1)
              }, o(a.title), 11, Nt)) : (n(), A(h, {
                key: a.id || a.route,
                to: a.route,
                class: u(["prism-sub-nav__panel-link", { "is-active": i.isItemActive(a) }]),
                onClick: t[1] || (t[1] = (b) => c.mobileExpanded = !1)
              }, {
                default: g(() => [
                  _(o(a.title), 1)
                ]),
                _: 2
              }, 1032, ["to", "class"]))
            ], 64))), 256))
          ])) : p("", !0)
        ]),
        _: 1
      }),
      l("nav", Lt, [
        (n(!0), s(v, null, k(r.items, (a, b) => (n(), s("button", {
          key: a.id || b,
          class: u(["prism-sub-nav__tab", { "is-active": i.isItemActive(a) || i.isParentActive(a) }]),
          onClick: (d) => i.onTabClick(a, b)
        }, [
          a.icon ? (n(), s("span", {
            key: 0,
            class: "prism-sub-nav__tab-icon",
            innerHTML: a.icon,
            "aria-hidden": "true"
          }, null, 8, Kt)) : p("", !0),
          l("span", Mt, o(a.title), 1)
        ], 10, Ht))), 128))
      ])
    ])
  ], 4);
}
const Xt = /* @__PURE__ */ m(mt, [["render", Rt], ["__scopeId", "data-v-bf4315a6"]]), Wt = [I, S, x, C, O, B, N, L, H, K, Xt], Ft = {
  install(e) {
    z(), Wt.forEach((t) => e.component(t.name, t));
  }
};
export {
  C as PrismAlertBanner,
  x as PrismAppBar,
  N as PrismAppNavDrawer,
  I as PrismButton,
  S as PrismHeroBanner,
  L as PrismPageTitle,
  H as PrismSectionTitle,
  O as PrismSplitSection,
  Xt as PrismSubNav,
  K as PrismSubSectionTitle,
  B as PrismTimeline,
  w as colors,
  Ft as default,
  z as injectColorTokens,
  Et as prismTheme
};
