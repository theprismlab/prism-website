import { openBlock as n, createElementBlock as i, normalizeClass as o, renderSlot as y, normalizeStyle as x, createElementVNode as s, toDisplayString as f, resolveComponent as X, createBlock as k, resolveDynamicComponent as K, mergeProps as L, withCtx as m, Fragment as b, renderList as _, createTextVNode as v, createCommentVNode as p, createVNode as W, Transition as D, withModifiers as H } from "vue";
const r0 = {
  red: {
    base: "#f44336",
    50: "#ffebee",
    100: "#ffcdd2",
    200: "#ef9a9a",
    300: "#e57373",
    400: "#ef5350",
    500: "#f44336",
    600: "#e53935",
    700: "#d32f2f",
    800: "#c62828",
    900: "#b71c1c",
    a100: "#ff8a80",
    a200: "#ff5252",
    a400: "#ff1744",
    a700: "#d50000",
    lighten5: "#ffebee",
    lighten4: "#ffcdd2",
    lighten3: "#ef9a9a",
    lighten2: "#e57373",
    lighten1: "#ef5350",
    darken1: "#e53935",
    darken2: "#d32f2f",
    darken3: "#c62828",
    darken4: "#b71c1c",
    accent1: "#ff8a80",
    accent2: "#ff5252",
    accent3: "#ff1744",
    accent4: "#d50000"
  },
  pink: {
    base: "#e91e63",
    50: "#fce4ec",
    100: "#f8bbd0",
    200: "#f48fb1",
    300: "#f06292",
    400: "#ec407a",
    500: "#e91e63",
    600: "#d81b60",
    700: "#c2185b",
    800: "#ad1457",
    900: "#880e4f",
    a100: "#ff80ab",
    a200: "#ff4081",
    a400: "#f50057",
    a700: "#c51162",
    lighten5: "#fce4ec",
    lighten4: "#f8bbd0",
    lighten3: "#f48fb1",
    lighten2: "#f06292",
    lighten1: "#ec407a",
    darken1: "#d81b60",
    darken2: "#c2185b",
    darken3: "#ad1457",
    darken4: "#880e4f",
    accent1: "#ff80ab",
    accent2: "#ff4081",
    accent3: "#f50057",
    accent4: "#c51162"
  },
  purple: {
    base: "#9c27b0",
    50: "#f3e5f5",
    100: "#e1bee7",
    200: "#ce93d8",
    300: "#ba68c8",
    400: "#ab47bc",
    500: "#9c27b0",
    600: "#8e24aa",
    700: "#7b1fa2",
    800: "#6a1b9a",
    900: "#4a148c",
    a100: "#ea80fc",
    a200: "#e040fb",
    a400: "#d500f9",
    a700: "#aa00ff",
    lighten5: "#f3e5f5",
    lighten4: "#e1bee7",
    lighten3: "#ce93d8",
    lighten2: "#ba68c8",
    lighten1: "#ab47bc",
    darken1: "#8e24aa",
    darken2: "#7b1fa2",
    darken3: "#6a1b9a",
    darken4: "#4a148c",
    accent1: "#ea80fc",
    accent2: "#e040fb",
    accent3: "#d500f9",
    accent4: "#aa00ff"
  },
  deepPurple: {
    base: "#673ab7",
    50: "#ede7f6",
    100: "#d1c4e9",
    200: "#b39ddb",
    300: "#9575cd",
    400: "#7e57c2",
    500: "#673ab7",
    600: "#5e35b1",
    700: "#512da8",
    800: "#4527a0",
    900: "#311b92",
    a100: "#b388ff",
    a200: "#7c4dff",
    a400: "#651fff",
    a700: "#6200ea",
    lighten5: "#ede7f6",
    lighten4: "#d1c4e9",
    lighten3: "#b39ddb",
    lighten2: "#9575cd",
    lighten1: "#7e57c2",
    darken1: "#5e35b1",
    darken2: "#512da8",
    darken3: "#4527a0",
    darken4: "#311b92",
    accent1: "#b388ff",
    accent2: "#7c4dff",
    accent3: "#651fff",
    accent4: "#6200ea"
  },
  indigo: {
    base: "#3f51b5",
    50: "#e8eaf6",
    100: "#c5cae9",
    200: "#9fa8da",
    300: "#7986cb",
    400: "#5c6bc0",
    500: "#3f51b5",
    600: "#3949ab",
    700: "#303f9f",
    800: "#283593",
    900: "#1a237e",
    a100: "#8c9eff",
    a200: "#536dfe",
    a400: "#3d5afe",
    a700: "#304ffe",
    lighten5: "#e8eaf6",
    lighten4: "#c5cae9",
    lighten3: "#9fa8da",
    lighten2: "#7986cb",
    lighten1: "#5c6bc0",
    darken1: "#3949ab",
    darken2: "#303f9f",
    darken3: "#283593",
    darken4: "#1a237e",
    accent1: "#8c9eff",
    accent2: "#536dfe",
    accent3: "#3d5afe",
    accent4: "#304ffe"
  },
  blue: {
    base: "#2196f3",
    50: "#e3f2fd",
    100: "#bbdefb",
    200: "#90caf9",
    300: "#64b5f6",
    400: "#42a5f5",
    500: "#2196f3",
    600: "#1e88e5",
    700: "#1976d2",
    800: "#1565c0",
    900: "#0d47a1",
    a100: "#82b1ff",
    a200: "#448aff",
    a400: "#2979ff",
    a700: "#2962ff",
    lighten5: "#e3f2fd",
    lighten4: "#bbdefb",
    lighten3: "#90caf9",
    lighten2: "#64b5f6",
    lighten1: "#42a5f5",
    darken1: "#1e88e5",
    darken2: "#1976d2",
    darken3: "#1565c0",
    darken4: "#0d47a1",
    accent1: "#82b1ff",
    accent2: "#448aff",
    accent3: "#2979ff",
    accent4: "#2962ff"
  },
  lightBlue: {
    base: "#03a9f4",
    50: "#e1f5fe",
    100: "#b3e5fc",
    200: "#81d4fa",
    300: "#4fc3f7",
    400: "#29b6f6",
    500: "#03a9f4",
    600: "#039be5",
    700: "#0288d1",
    800: "#0277bd",
    900: "#01579b",
    a100: "#80d8ff",
    a200: "#40c4ff",
    a400: "#00b0ff",
    a700: "#0091ea",
    lighten5: "#e1f5fe",
    lighten4: "#b3e5fc",
    lighten3: "#81d4fa",
    lighten2: "#4fc3f7",
    lighten1: "#29b6f6",
    darken1: "#039be5",
    darken2: "#0288d1",
    darken3: "#0277bd",
    darken4: "#01579b",
    accent1: "#80d8ff",
    accent2: "#40c4ff",
    accent3: "#00b0ff",
    accent4: "#0091ea"
  },
  cyan: {
    base: "#00bcd4",
    50: "#e0f7fa",
    100: "#b2ebf2",
    200: "#80deea",
    300: "#4dd0e1",
    400: "#26c6da",
    500: "#00bcd4",
    600: "#00acc1",
    700: "#0097a7",
    800: "#00838f",
    900: "#006064",
    a100: "#84ffff",
    a200: "#18ffff",
    a400: "#00e5ff",
    a700: "#00b8d4",
    lighten5: "#e0f7fa",
    lighten4: "#b2ebf2",
    lighten3: "#80deea",
    lighten2: "#4dd0e1",
    lighten1: "#26c6da",
    darken1: "#00acc1",
    darken2: "#0097a7",
    darken3: "#00838f",
    darken4: "#006064",
    accent1: "#84ffff",
    accent2: "#18ffff",
    accent3: "#00e5ff",
    accent4: "#00b8d4"
  },
  teal: {
    base: "#009688",
    50: "#e0f2f1",
    100: "#b2dfdb",
    200: "#80cbc4",
    300: "#4db6ac",
    400: "#26a69a",
    500: "#009688",
    600: "#00897b",
    700: "#00796b",
    800: "#00695c",
    900: "#004d40",
    a100: "#a7ffeb",
    a200: "#64ffda",
    a400: "#1de9b6",
    a700: "#00bfa5",
    lighten5: "#e0f2f1",
    lighten4: "#b2dfdb",
    lighten3: "#80cbc4",
    lighten2: "#4db6ac",
    lighten1: "#26a69a",
    darken1: "#00897b",
    darken2: "#00796b",
    darken3: "#00695c",
    darken4: "#004d40",
    accent1: "#a7ffeb",
    accent2: "#64ffda",
    accent3: "#1de9b6",
    accent4: "#00bfa5"
  },
  green: {
    base: "#4caf50",
    50: "#e8f5e9",
    100: "#c8e6c9",
    200: "#a5d6a7",
    300: "#81c784",
    400: "#66bb6a",
    500: "#4caf50",
    600: "#43a047",
    700: "#388e3c",
    800: "#2e7d32",
    900: "#1b5e20",
    a100: "#b9f6ca",
    a200: "#69f0ae",
    a400: "#00e676",
    a700: "#00c853",
    lighten5: "#e8f5e9",
    lighten4: "#c8e6c9",
    lighten3: "#a5d6a7",
    lighten2: "#81c784",
    lighten1: "#66bb6a",
    darken1: "#43a047",
    darken2: "#388e3c",
    darken3: "#2e7d32",
    darken4: "#1b5e20",
    accent1: "#b9f6ca",
    accent2: "#69f0ae",
    accent3: "#00e676",
    accent4: "#00c853"
  },
  lightGreen: {
    base: "#8bc34a",
    50: "#f1f8e9",
    100: "#dcedc8",
    200: "#c5e1a5",
    300: "#aed581",
    400: "#9ccc65",
    500: "#8bc34a",
    600: "#7cb342",
    700: "#689f38",
    800: "#558b2f",
    900: "#33691e",
    a100: "#ccff90",
    a200: "#b2ff59",
    a400: "#76ff03",
    a700: "#64dd17",
    lighten5: "#f1f8e9",
    lighten4: "#dcedc8",
    lighten3: "#c5e1a5",
    lighten2: "#aed581",
    lighten1: "#9ccc65",
    darken1: "#7cb342",
    darken2: "#689f38",
    darken3: "#558b2f",
    darken4: "#33691e",
    accent1: "#ccff90",
    accent2: "#b2ff59",
    accent3: "#76ff03",
    accent4: "#64dd17"
  },
  lime: {
    base: "#cddc39",
    50: "#f9fbe7",
    100: "#f0f4c3",
    200: "#e6ee9c",
    300: "#dce775",
    400: "#d4e157",
    500: "#cddc39",
    600: "#c0ca33",
    700: "#afb42b",
    800: "#9e9d24",
    900: "#827717",
    a100: "#f4ff81",
    a200: "#eeff41",
    a400: "#c6ff00",
    a700: "#aeea00",
    lighten5: "#f9fbe7",
    lighten4: "#f0f4c3",
    lighten3: "#e6ee9c",
    lighten2: "#dce775",
    lighten1: "#d4e157",
    darken1: "#c0ca33",
    darken2: "#afb42b",
    darken3: "#9e9d24",
    darken4: "#827717",
    accent1: "#f4ff81",
    accent2: "#eeff41",
    accent3: "#c6ff00",
    accent4: "#aeea00"
  },
  yellow: {
    base: "#ffeb3b",
    50: "#fffde7",
    100: "#fff9c4",
    200: "#fff59d",
    300: "#fff176",
    400: "#ffee58",
    500: "#ffeb3b",
    600: "#fdd835",
    700: "#fbc02d",
    800: "#f9a825",
    900: "#f57f17",
    a100: "#ffff8d",
    a200: "#ffff00",
    a400: "#ffea00",
    a700: "#ffd600",
    lighten5: "#fffde7",
    lighten4: "#fff9c4",
    lighten3: "#fff59d",
    lighten2: "#fff176",
    lighten1: "#ffee58",
    darken1: "#fdd835",
    darken2: "#fbc02d",
    darken3: "#f9a825",
    darken4: "#f57f17",
    accent1: "#ffff8d",
    accent2: "#ffff00",
    accent3: "#ffea00",
    accent4: "#ffd600"
  },
  amber: {
    base: "#ffc107",
    50: "#fff8e1",
    100: "#ffecb3",
    200: "#ffe082",
    300: "#ffd54f",
    400: "#ffca28",
    500: "#ffc107",
    600: "#ffb300",
    700: "#ffa000",
    800: "#ff8f00",
    900: "#ff6f00",
    a100: "#ffe57f",
    a200: "#ffd740",
    a400: "#ffc400",
    a700: "#ffab00",
    lighten5: "#fff8e1",
    lighten4: "#ffecb3",
    lighten3: "#ffe082",
    lighten2: "#ffd54f",
    lighten1: "#ffca28",
    darken1: "#ffb300",
    darken2: "#ffa000",
    darken3: "#ff8f00",
    darken4: "#ff6f00",
    accent1: "#ffe57f",
    accent2: "#ffd740",
    accent3: "#ffc400",
    accent4: "#ffab00"
  },
  orange: {
    base: "#ff9800",
    50: "#fff3e0",
    100: "#ffe0b2",
    200: "#ffcc80",
    300: "#ffb74d",
    400: "#ffa726",
    500: "#ff9800",
    600: "#fb8c00",
    700: "#f57c00",
    800: "#ef6c00",
    900: "#e65100",
    a100: "#ffd180",
    a200: "#ffab40",
    a400: "#ff9100",
    a700: "#ff6d00",
    lighten5: "#fff3e0",
    lighten4: "#ffe0b2",
    lighten3: "#ffcc80",
    lighten2: "#ffb74d",
    lighten1: "#ffa726",
    darken1: "#fb8c00",
    darken2: "#f57c00",
    darken3: "#ef6c00",
    darken4: "#e65100",
    accent1: "#ffd180",
    accent2: "#ffab40",
    accent3: "#ff9100",
    accent4: "#ff6d00"
  },
  deepOrange: {
    base: "#ff5722",
    50: "#fbe9e7",
    100: "#ffccbc",
    200: "#ffab91",
    300: "#ff8a65",
    400: "#ff7043",
    500: "#ff5722",
    600: "#f4511e",
    700: "#e64a19",
    800: "#d84315",
    900: "#bf360c",
    a100: "#ff9e80",
    a200: "#ff6e40",
    a400: "#ff3d00",
    a700: "#dd2c00",
    lighten5: "#fbe9e7",
    lighten4: "#ffccbc",
    lighten3: "#ffab91",
    lighten2: "#ff8a65",
    lighten1: "#ff7043",
    darken1: "#f4511e",
    darken2: "#e64a19",
    darken3: "#d84315",
    darken4: "#bf360c",
    accent1: "#ff9e80",
    accent2: "#ff6e40",
    accent3: "#ff3d00",
    accent4: "#dd2c00"
  },
  brown: {
    base: "#795548",
    50: "#efebe9",
    100: "#d7ccc8",
    200: "#bcaaa4",
    300: "#a1887f",
    400: "#8d6e63",
    500: "#795548",
    600: "#6d4c41",
    700: "#5d4037",
    800: "#4e342e",
    900: "#3e2723",
    lighten5: "#efebe9",
    lighten4: "#d7ccc8",
    lighten3: "#bcaaa4",
    lighten2: "#a1887f",
    lighten1: "#8d6e63",
    darken1: "#6d4c41",
    darken2: "#5d4037",
    darken3: "#4e342e",
    darken4: "#3e2723"
  },
  grey: {
    base: "#9e9e9e",
    50: "#fafafa",
    100: "#f5f5f5",
    200: "#eeeeee",
    300: "#e0e0e0",
    400: "#bdbdbd",
    500: "#9e9e9e",
    600: "#757575",
    700: "#616161",
    800: "#424242",
    900: "#212121",
    lighten5: "#fafafa",
    lighten4: "#f5f5f5",
    lighten3: "#eeeeee",
    lighten2: "#e0e0e0",
    lighten1: "#bdbdbd",
    darken1: "#757575",
    darken2: "#616161",
    darken3: "#424242",
    darken4: "#212121"
  },
  blueGrey: {
    base: "#607d8b",
    50: "#eceff1",
    100: "#cfd8dc",
    200: "#b0bec5",
    300: "#90a4ae",
    400: "#78909c",
    500: "#607d8b",
    600: "#546e7a",
    700: "#455a64",
    800: "#37474f",
    900: "#263238",
    lighten5: "#eceff1",
    lighten4: "#cfd8dc",
    lighten3: "#b0bec5",
    lighten2: "#90a4ae",
    lighten1: "#78909c",
    darken1: "#546e7a",
    darken2: "#455a64",
    darken3: "#37474f",
    darken4: "#263238"
  },
  shades: {
    black: "#000000",
    white: "#ffffff",
    transparent: "transparent"
  }
};
const A = (e, t) => {
  const r = e.__vccOpts || e;
  for (const [u, d] of t)
    r[u] = d;
  return r;
}, M = {
  name: "PrismButton",
  props: {
    variant: {
      type: String,
      default: "primary",
      validator: (e) => [
        "primary",
        "secondary",
        "ghost",
        "danger",
        "gradient-primary",
        "gradient-sunset",
        "gradient-ocean",
        "gradient-forest",
        "gradient-aurora",
        "gradient-fire"
      ].includes(e)
    },
    size: {
      type: String,
      default: "md",
      validator: (e) => ["sm", "md", "lg"].includes(e)
    },
    type: {
      type: String,
      default: "button"
    },
    disabled: {
      type: Boolean,
      default: !1
    }
  },
  methods: {
    onClick(e) {
      this.$emit("click", e);
    }
  }
}, F = ["type", "disabled"];
function J(e, t, r, u, d, l) {
  return n(), i("button", {
    type: r.type,
    class: o(["prism-button", `prism-button--${r.variant}`, `prism-button--${r.size}`, { "is-disabled": r.disabled }]),
    disabled: r.disabled,
    onClick: t[0] || (t[0] = (...g) => l.onClick && l.onClick(...g))
  }, [
    y(e.$slots, "default", {}, void 0, !0)
  ], 10, F);
}
const w = /* @__PURE__ */ A(M, [["render", J], ["__scopeId", "data-v-8e95ccfa"]]);
w.install = function(t) {
  t.component(w.name, w);
};
const Q = {
  name: "PrismHeroBanner",
  props: {
    image: {
      type: String,
      default: ""
    },
    title: {
      type: String,
      default: ""
    },
    body: {
      type: String,
      default: ""
    },
    variant: {
      type: String,
      default: "primary"
    }
  }
}, U = { class: "hero__content" }, z = { class: "hero__title" }, j = { class: "hero__description" };
function E(e, t, r, u, d, l) {
  return n(), i("section", {
    class: o(["hero__container", `hero__container--${r.variant} ${r.image ? "hero__container--image" : ""}`]),
    style: x(r.image ? {
      backgroundImage: `url(${r.image})`,
      backgroundSize: "cover",
      backgroundPosition: "center"
    } : {})
  }, [
    s("div", U, [
      s("h2", z, f(r.title), 1),
      s("div", j, [
        s("p", null, f(r.body), 1),
        y(e.$slots, "default", {}, void 0, !0)
      ])
    ])
  ], 6);
}
const I = /* @__PURE__ */ A(Q, [["render", E], ["__scopeId", "data-v-ffdf902f"]]);
I.install = function(t) {
  t.component(I.name, I);
};
const V = "data:image/png;base64,";
function C() {
  try {
    const e = JSON.parse(localStorage.getItem("user_info"));
    return !!e && !e.isAnonymous;
  } catch {
    return !1;
  }
}
const q = { marketing: "", portal: "/portal" }, Y = [
  { title: "About us", children: [
    { title: "About PRISM", route: "/about-us/about-prism", id: "about-prism", site: "marketing" },
    { title: "Team", route: "/about-us/team", id: "team", site: "marketing" }
  ] },
  { title: "Resources", children: [
    { title: "Publications", route: "/publications", id: "publications", site: "marketing" },
    { title: "FAQs", route: "/faqs", id: "faqs", site: "marketing" },
    { title: "Webinars", route: "/webinars", id: "webinars", site: "marketing" }
  ] },
  { title: "Consortium Screens", children: [
    { title: "Cell line collection", route: "/consortium-screens/cell-line-collection", id: "cell-line-collection", site: "marketing" },
    { title: "Assays", route: "/consortium-screens/assays", id: "assays", site: "marketing" },
    { title: "Data Analysis", route: "/consortium-screens/data-analysis", id: "data-analysis", site: "marketing" },
    { title: "Deliverables", route: "/consortium-screens/deliverables", id: "deliverables", site: "marketing" },
    { title: "Collaborating", route: "/consortium-screens/collaborating", id: "collaborating", site: "marketing" },
    { title: "Submissions", route: "/submissions", id: "submissions", site: "marketing" }
  ] },
  { title: "Contact us", route: "/contact-us", id: "contact-us", site: "marketing" },
  { title: "Portal", route: "/portal", id: "portal", matchPrefix: !0, site: "portal" }
], Z = { route: "/", site: "marketing" }, G = { route: "/portal/log-in", site: "portal" };
function S(e, t, r) {
  if (e.children)
    return {
      ...e,
      children: e.children.map((u) => S(u, t, r))
    };
  if (!e.route)
    return e;
  if (!e.site || e.site === t) {
    const u = q[t] || "", d = u && e.route.startsWith(u) ? e.route.slice(u.length) || "/" : e.route;
    return { ...e, route: d, external: !1 };
  }
  return { ...e, route: `${r}${e.route}`, external: !0 };
}
const $ = {
  name: "PrismAppBar",
  props: {
    site: {
      type: String,
      default: "marketing",
      validator: (e) => ["marketing", "portal"].includes(e)
    },
    baseUrl: {
      type: String,
      default: ""
    }
  },
  data() {
    return {
      prismLogo: V,
      drawerOpen: !1,
      openGroups: [],
      isLoggedIn: C()
    };
  },
  mounted() {
    window.addEventListener("storage", this._onAuthChange);
  },
  beforeDestroy() {
    window.removeEventListener("storage", this._onAuthChange);
  },
  computed: {
    resolvedLogo() {
      return S(Z, this.site, this.baseUrl);
    },
    resolvedItems() {
      return Y.map((e) => S(e, this.site, this.baseUrl));
    },
    resolvedLoginItem() {
      return S(G, this.site, this.baseUrl);
    }
  },
  methods: {
    _onAuthChange(e) {
      (e.key === "user_info" || e.key === null) && (this.isLoggedIn = C());
    },
    _currentPath() {
      var e;
      return ((e = this.$route) == null ? void 0 : e.path) ?? window.location.pathname;
    },
    _matchPath(e) {
      if (!e)
        return !1;
      try {
        return new URL(e, window.location.href).pathname === this._currentPath();
      } catch {
        return e === this._currentPath();
      }
    },
    isParentActive(e) {
      return !e.children || !e.children.length ? !1 : e.children.some((t) => this._matchPath(t.route));
    },
    isItemActive(e) {
      if (e.children && e.children.length)
        return this.isParentActive(e);
      if (e.matchPrefix)
        try {
          const t = new URL(e.route, window.location.href).pathname;
          return window.location.pathname.startsWith(t);
        } catch {
          return window.location.pathname.startsWith(e.route);
        }
      return this._matchPath(e.route);
    },
    navigateTo(e) {
      this.drawerOpen = !1, this.$router.push(e).catch(() => {
      });
    },
    toggleGroup(e) {
      this.openGroups.includes(e) ? this.openGroups = this.openGroups.filter((t) => t !== e) : this.openGroups.push(e);
    }
  }
}, ee = { class: "prism-app-bar-root" }, te = { class: "prism-app-bar" }, ae = { class: "prism-app-bar__inner" }, ne = ["src"], re = ["aria-expanded"], ie = {
  class: "prism-app-bar__desktop-nav",
  "aria-label": "Main navigation"
}, se = ["href"], le = {
  key: 3,
  class: "prism-app-bar__dropdown"
}, de = ["href"], fe = { class: "prism-app-bar__cta" }, ce = {
  key: 0,
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24",
  fill: "currentColor",
  width: "16",
  height: "16",
  "aria-hidden": "true"
}, oe = {
  key: 0,
  class: "prism-nav-drawer",
  "aria-label": "Mobile navigation"
}, pe = ["href"], ue = ["href", "onClick"], ge = ["onClick"], be = {
  key: 0,
  class: "prism-nav-drawer__children"
}, he = ["href"], ve = ["href", "onClick"], me = { class: "prism-nav-drawer__actions" }, Ae = {
  key: 0,
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 24 24",
  fill: "currentColor",
  width: "16",
  height: "16",
  "aria-hidden": "true"
};
function ke(e, t, r, u, d, l) {
  const g = X("router-link");
  return n(), i("div", ee, [
    s("header", te, [
      s("div", ae, [
        (n(), k(K(l.resolvedLogo.external ? "a" : "router-link"), L(l.resolvedLogo.external ? { href: l.resolvedLogo.route } : { to: l.resolvedLogo.route }, { class: "prism-app-bar__logo-link" }), {
          default: m(() => [
            s("img", {
              src: d.prismLogo,
              alt: "PRISM",
              class: "prism-app-bar__logo-img"
            }, null, 8, ne)
          ]),
          _: 1
        }, 16)),
        s("button", {
          class: "prism-app-bar__hamburger",
          "aria-expanded": String(d.drawerOpen),
          "aria-label": "Toggle navigation",
          onClick: t[0] || (t[0] = (a) => d.drawerOpen = !d.drawerOpen)
        }, [...t[5] || (t[5] = [
          s("span", null, null, -1),
          s("span", null, null, -1),
          s("span", null, null, -1)
        ])], 8, re),
        s("nav", ie, [
          (n(!0), i(b, null, _(l.resolvedItems, (a, h) => (n(), i("div", {
            key: h,
            class: o(["prism-app-bar__nav-item", { "prism-app-bar__nav-item--has-children": a.children && a.children.length }])
          }, [
            a.children && a.children.length ? (n(), i("button", {
              key: 0,
              type: "button",
              class: o(["prism-app-bar__nav-link", { "is-active": l.isParentActive(a) }])
            }, [
              v(f(a.title) + " ", 1),
              t[6] || (t[6] = s("svg", {
                class: "prism-app-bar__chevron",
                xmlns: "http://www.w3.org/2000/svg",
                viewBox: "0 0 20 20",
                fill: "currentColor",
                width: "16",
                height: "16",
                "aria-hidden": "true"
              }, [
                s("path", {
                  "fill-rule": "evenodd",
                  d: "M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z",
                  "clip-rule": "evenodd"
                })
              ], -1))
            ], 2)) : a.external ? (n(), i("a", {
              key: 1,
              href: a.route,
              class: o(["prism-app-bar__nav-link", { "is-active": l.isItemActive(a) }])
            }, f(a.title), 11, se)) : (n(), k(g, {
              key: 2,
              to: a.route,
              class: o(["prism-app-bar__nav-link", { "is-active": l.isItemActive(a) }])
            }, {
              default: m(() => [
                v(f(a.title), 1)
              ]),
              _: 2
            }, 1032, ["to", "class"])),
            a.children && a.children.length ? (n(), i("div", le, [
              (n(!0), i(b, null, _(a.children, (c) => (n(), i(b, null, [
                c.external ? (n(), i("a", {
                  key: `ext-${c.id}`,
                  href: c.route,
                  class: o(["prism-app-bar__dropdown-link", { "is-active": e.$route && e.$route.path === c.route }])
                }, f(c.title), 11, de)) : (n(), k(g, {
                  key: c.id,
                  to: c.route,
                  class: o(["prism-app-bar__dropdown-link", { "is-active": e.$route && e.$route.path === c.route }])
                }, {
                  default: m(() => [
                    v(f(c.title), 1)
                  ]),
                  _: 2
                }, 1032, ["to", "class"]))
              ], 64))), 256))
            ])) : p("", !0)
          ], 2))), 128)),
          s("div", fe, [
            (n(), k(K(l.resolvedLoginItem.external ? "a" : "router-link"), L(l.resolvedLoginItem.external ? { href: l.resolvedLoginItem.route } : { to: l.resolvedLoginItem.route }, { class: "prism-app-bar__login-btn" }), {
              default: m(() => [
                d.isLoggedIn ? (n(), i("svg", ce, [...t[7] || (t[7] = [
                  s("path", { d: "M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z" }, null, -1)
                ])])) : p("", !0),
                v(" " + f(d.isLoggedIn ? "" : "Log In"), 1)
              ]),
              _: 1
            }, 16))
          ])
        ])
      ])
    ]),
    d.drawerOpen ? (n(), i("div", {
      key: 0,
      class: "prism-nav-drawer__backdrop",
      "aria-hidden": "true",
      onClick: t[1] || (t[1] = (a) => d.drawerOpen = !1)
    })) : p("", !0),
    W(D, { name: "prism-drawer-slide" }, {
      default: m(() => [
        d.drawerOpen ? (n(), i("nav", oe, [
          (n(!0), i(b, null, _(l.resolvedItems, (a, h) => (n(), i("div", {
            key: h,
            class: "prism-nav-drawer__item"
          }, [
            !a.children || a.children.length === 0 ? (n(), i(b, { key: 0 }, [
              a.external ? (n(), i("a", {
                key: `ext-${a.id}`,
                href: a.route,
                class: o(["prism-nav-drawer__link", { "is-active": e.$route && e.$route.path === a.route }]),
                onClick: t[2] || (t[2] = (c) => d.drawerOpen = !1)
              }, f(a.title), 11, pe)) : (n(), i("a", {
                key: a.id,
                href: a.route,
                class: o(["prism-nav-drawer__link", { "is-active": e.$route && e.$route.path === a.route }]),
                onClick: H((c) => l.navigateTo(a.route), ["prevent"])
              }, f(a.title), 11, ue))
            ], 64)) : (n(), i(b, { key: 1 }, [
              s("button", {
                class: o([
                  "prism-nav-drawer__group-btn",
                  { "is-active": l.isParentActive(a), "is-open": d.openGroups.includes(h) }
                ]),
                onClick: (c) => l.toggleGroup(h)
              }, [
                v(f(a.title) + " ", 1),
                t[8] || (t[8] = s("svg", {
                  class: "prism-nav-drawer__chevron",
                  xmlns: "http://www.w3.org/2000/svg",
                  viewBox: "0 0 20 20",
                  fill: "currentColor",
                  width: "16",
                  height: "16",
                  "aria-hidden": "true"
                }, [
                  s("path", {
                    "fill-rule": "evenodd",
                    d: "M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z",
                    "clip-rule": "evenodd"
                  })
                ], -1))
              ], 10, ge),
              d.openGroups.includes(h) ? (n(), i("div", be, [
                (n(!0), i(b, null, _(a.children, (c) => (n(), i(b, null, [
                  c.external ? (n(), i("a", {
                    key: `ext-${c.id}`,
                    href: c.route,
                    class: o(["prism-nav-drawer__child-link", { "is-active": e.$route && e.$route.path === c.route }]),
                    onClick: t[3] || (t[3] = (R) => d.drawerOpen = !1)
                  }, f(c.title), 11, he)) : (n(), i("a", {
                    key: c.id,
                    href: c.route,
                    class: o(["prism-nav-drawer__child-link", { "is-active": e.$route && e.$route.path === c.route }]),
                    onClick: H((R) => l.navigateTo(c.route), ["prevent"])
                  }, f(c.title), 11, ve))
                ], 64))), 256))
              ])) : p("", !0)
            ], 64))
          ]))), 128)),
          s("div", me, [
            (n(), k(K(l.resolvedLoginItem.external ? "a" : "router-link"), L(l.resolvedLoginItem.external ? { href: l.resolvedLoginItem.route } : { to: l.resolvedLoginItem.route }, {
              class: "prism-app-bar__login-btn",
              onClick: t[4] || (t[4] = (a) => d.drawerOpen = !1)
            }), {
              default: m(() => [
                d.isLoggedIn ? (n(), i("svg", Ae, [...t[9] || (t[9] = [
                  s("path", { d: "M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z" }, null, -1)
                ])])) : p("", !0),
                v(" " + f(d.isLoggedIn ? "Account" : "Log In"), 1)
              ]),
              _: 1
            }, 16))
          ])
        ])) : p("", !0)
      ]),
      _: 1
    })
  ]);
}
const T = /* @__PURE__ */ A($, [["render", ke], ["__scopeId", "data-v-b1302ba2"]]);
T.install = function(t) {
  t.component(T.name, T);
};
const _e = {
  name: "PrismAlertBanner",
  props: {
    variant: {
      type: String,
      default: "info",
      validator: (e) => ["info", "success", "warning", "error", "neutral"].includes(e)
    },
    title: {
      type: String,
      default: ""
    },
    message: {
      type: String,
      default: ""
    },
    dismissible: {
      type: Boolean,
      default: !1
    },
    bordered: {
      type: Boolean,
      default: !1
    }
  },
  data() {
    return { visible: !0 };
  },
  methods: {
    dismiss() {
      this.visible = !1, this.$emit("dismiss");
    }
  }
}, ye = {
  class: "prism-alert__icon",
  "aria-hidden": "true"
}, we = {
  key: 0,
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 20 20",
  fill: "currentColor",
  width: "20",
  height: "20"
}, Ie = {
  key: 1,
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 20 20",
  fill: "currentColor",
  width: "20",
  height: "20"
}, Se = {
  key: 2,
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 20 20",
  fill: "currentColor",
  width: "20",
  height: "20"
}, Te = {
  key: 3,
  xmlns: "http://www.w3.org/2000/svg",
  viewBox: "0 0 20 20",
  fill: "currentColor",
  width: "20",
  height: "20"
}, Pe = { class: "prism-alert__body" }, Oe = {
  key: 0,
  class: "prism-alert__title"
}, Be = { class: "prism-alert__message" };
function Ne(e, t, r, u, d, l) {
  return d.visible ? (n(), i("div", {
    key: 0,
    class: o(["prism-alert", `prism-alert--${r.variant}`, { "prism-alert--bordered": r.bordered }]),
    role: "alert"
  }, [
    s("span", ye, [
      r.variant === "success" ? (n(), i("svg", we, [...t[1] || (t[1] = [
        s("path", {
          "fill-rule": "evenodd",
          d: "M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z",
          "clip-rule": "evenodd"
        }, null, -1)
      ])])) : r.variant === "warning" ? (n(), i("svg", Ie, [...t[2] || (t[2] = [
        s("path", {
          "fill-rule": "evenodd",
          d: "M8.485 2.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.347 0-2.189-1.458-1.515-2.625L8.485 2.495zM10 5a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 5zm0 9a1 1 0 100-2 1 1 0 000 2z",
          "clip-rule": "evenodd"
        }, null, -1)
      ])])) : r.variant === "error" ? (n(), i("svg", Se, [...t[3] || (t[3] = [
        s("path", {
          "fill-rule": "evenodd",
          d: "M10 18a8 8 0 100-16 8 8 0 000 16zM8.28 7.22a.75.75 0 00-1.06 1.06L8.94 10l-1.72 1.72a.75.75 0 101.06 1.06L10 11.06l1.72 1.72a.75.75 0 101.06-1.06L11.06 10l1.72-1.72a.75.75 0 00-1.06-1.06L10 8.94 8.28 7.22z",
          "clip-rule": "evenodd"
        }, null, -1)
      ])])) : (n(), i("svg", Te, [...t[4] || (t[4] = [
        s("path", {
          "fill-rule": "evenodd",
          d: "M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.253a.25.25 0 01.244.304l-.459 2.066A1.75 1.75 0 0010.747 15H11a.75.75 0 000-1.5h-.253a.25.25 0 01-.244-.304l.459-2.066A1.75 1.75 0 009.253 9H9z",
          "clip-rule": "evenodd"
        }, null, -1)
      ])]))
    ]),
    s("div", Pe, [
      r.title ? (n(), i("p", Oe, f(r.title), 1)) : p("", !0),
      s("div", Be, [
        y(e.$slots, "default", {}, () => [
          v(f(r.message), 1)
        ], !0)
      ])
    ]),
    r.dismissible ? (n(), i("button", {
      key: 0,
      class: "prism-alert__close",
      "aria-label": "Dismiss",
      onClick: t[0] || (t[0] = (...g) => l.dismiss && l.dismiss(...g))
    }, [...t[5] || (t[5] = [
      s("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 20 20",
        fill: "currentColor",
        width: "16",
        height: "16",
        "aria-hidden": "true"
      }, [
        s("path", { d: "M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" })
      ], -1)
    ])])) : p("", !0)
  ], 2)) : p("", !0);
}
const P = /* @__PURE__ */ A(_e, [["render", Ne], ["__scopeId", "data-v-8d5eb52d"]]);
P.install = function(t) {
  t.component(P.name, P);
};
const Ke = {
  name: "PrismSplitSection",
  props: {
    image: {
      type: String,
      default: ""
    },
    imageAlt: {
      type: String,
      default: ""
    },
    eyebrow: {
      type: String,
      default: ""
    },
    title: {
      type: String,
      default: ""
    },
    body: {
      type: String,
      default: ""
    },
    reverse: {
      type: Boolean,
      default: !1
    }
  }
}, Le = { class: "prism-split__image-col" }, He = ["src", "alt"], Ce = { class: "prism-split__content-col" }, xe = {
  key: 0,
  class: "prism-split__eyebrow"
}, Xe = {
  key: 1,
  class: "prism-split__title"
}, Re = {
  key: 2,
  class: "prism-split__body"
}, We = {
  key: 3,
  class: "prism-split__actions"
};
function De(e, t, r, u, d, l) {
  return n(), i("section", {
    class: o(["prism-split", { "prism-split--reverse": r.reverse }])
  }, [
    s("div", Le, [
      r.image ? (n(), i("img", {
        key: 0,
        src: r.image,
        alt: r.imageAlt,
        class: "prism-split__image"
      }, null, 8, He)) : p("", !0),
      y(e.$slots, "image", {}, void 0, !0)
    ]),
    s("div", Ce, [
      r.eyebrow ? (n(), i("p", xe, f(r.eyebrow), 1)) : p("", !0),
      r.title ? (n(), i("h2", Xe, f(r.title), 1)) : p("", !0),
      r.body || e.$slots.default ? (n(), i("div", Re, [
        y(e.$slots, "default", {}, () => [
          v(f(r.body), 1)
        ], !0)
      ])) : p("", !0),
      e.$slots.actions ? (n(), i("div", We, [
        y(e.$slots, "actions", {}, void 0, !0)
      ])) : p("", !0)
    ])
  ], 2);
}
const O = /* @__PURE__ */ A(Ke, [["render", De], ["__scopeId", "data-v-46e54712"]]);
O.install = function(t) {
  t.component(O.name, O);
};
const Me = {
  name: "PrismTimeline",
  props: {
    steps: {
      type: Array,
      required: !0
    },
    activeStep: {
      type: Number,
      default: -1
    },
    variant: {
      type: String,
      default: "vertical",
      validator: (e) => ["vertical", "horizontal"].includes(e)
    }
  }
}, Fe = { class: "prism-timeline__marker" }, Je = {
  key: 0,
  class: "prism-timeline__check",
  "aria-hidden": "true"
}, Qe = {
  key: 1,
  class: "prism-timeline__number"
}, Ue = { class: "prism-timeline__content" }, ze = { class: "prism-timeline__title" }, je = {
  key: 0,
  class: "prism-timeline__caption"
};
function Ee(e, t, r, u, d, l) {
  return n(), i("ol", {
    class: o(["prism-timeline", `prism-timeline--${r.variant}`])
  }, [
    (n(!0), i(b, null, _(r.steps, (g, a) => (n(), i("li", {
      key: a,
      class: o(["prism-timeline__step", { "prism-timeline__step--active": a === r.activeStep, "prism-timeline__step--completed": a < r.activeStep }])
    }, [
      t[1] || (t[1] = s("div", {
        class: "prism-timeline__connector",
        "aria-hidden": "true"
      }, null, -1)),
      s("div", Fe, [
        a < r.activeStep ? (n(), i("span", Je, [...t[0] || (t[0] = [
          s("svg", {
            xmlns: "http://www.w3.org/2000/svg",
            viewBox: "0 0 20 20",
            fill: "currentColor",
            width: "14",
            height: "14"
          }, [
            s("path", {
              "fill-rule": "evenodd",
              d: "M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z",
              "clip-rule": "evenodd"
            })
          ], -1)
        ])])) : (n(), i("span", Qe, f(a + 1), 1))
      ]),
      s("div", Ue, [
        s("p", ze, f(g.title), 1),
        g.caption ? (n(), i("p", je, f(g.caption), 1)) : p("", !0)
      ])
    ], 2))), 128))
  ], 2);
}
const B = /* @__PURE__ */ A(Me, [["render", Ee], ["__scopeId", "data-v-6d8972d2"]]);
B.install = function(t) {
  t.component(B.name, B);
};
const Ve = {
  name: "PrismAppNavDrawer",
  props: {
    items: {
      type: Array,
      default: () => []
    },
    top: {
      type: String,
      default: "64px"
    }
  },
  data() {
    return {
      isExpanded: !1
    };
  },
  methods: {
    isActive(e) {
      var r;
      return (((r = this.$route) == null ? void 0 : r.path) ?? "") === e.route;
    }
  }
}, qe = { class: "prism-app-nav-drawer__list" }, Ye = ["href", "title"], Ze = ["innerHTML"], Ge = { class: "prism-app-nav-drawer__label" }, $e = ["innerHTML"], e0 = { class: "prism-app-nav-drawer__label" };
function t0(e, t, r, u, d, l) {
  const g = X("router-link");
  return n(), i("nav", {
    class: o(["prism-app-nav-drawer", { "is-expanded": d.isExpanded }]),
    style: x({ top: r.top }),
    onMouseenter: t[0] || (t[0] = (a) => d.isExpanded = !0),
    onMouseleave: t[1] || (t[1] = (a) => d.isExpanded = !1),
    "aria-label": "Side navigation"
  }, [
    s("ul", qe, [
      (n(!0), i(b, null, _(r.items, (a, h) => (n(), i("li", {
        key: h,
        class: "prism-app-nav-drawer__item"
      }, [
        a.external ? (n(), i("a", {
          key: 0,
          href: a.route,
          title: a.title,
          class: o(["prism-app-nav-drawer__link", { "is-active": l.isActive(a) }])
        }, [
          s("span", {
            class: "prism-app-nav-drawer__icon",
            innerHTML: a.icon,
            "aria-hidden": "true"
          }, null, 8, Ze),
          s("span", Ge, f(a.title), 1)
        ], 10, Ye)) : (n(), k(g, {
          key: 1,
          to: a.route,
          title: a.title,
          class: o(["prism-app-nav-drawer__link", { "is-active": e.$route && e.$route.path === a.route }])
        }, {
          default: m(() => [
            s("span", {
              class: "prism-app-nav-drawer__icon",
              innerHTML: a.icon,
              "aria-hidden": "true"
            }, null, 8, $e),
            s("span", e0, f(a.title), 1)
          ]),
          _: 2
        }, 1032, ["to", "title", "class"]))
      ]))), 128))
    ])
  ], 38);
}
const N = /* @__PURE__ */ A(Ve, [["render", t0], ["__scopeId", "data-v-b4c93d9a"]]);
N.install = function(t) {
  t.component(N.name, N);
};
const a0 = [w, I, T, P, O, B, N], i0 = {
  install(e) {
    a0.forEach((t) => e.component(t.name, t));
  }
};
export {
  P as PrismAlertBanner,
  T as PrismAppBar,
  N as PrismAppNavDrawer,
  w as PrismButton,
  I as PrismHeroBanner,
  O as PrismSplitSection,
  B as PrismTimeline,
  r0 as colors,
  i0 as default
};
