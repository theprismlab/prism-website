...
function computeGridIndices(raw) {
  const cellLineGroups = d3
    .groups(raw, (d) => d.ccle_name)
...
const xScale = d3
  .scaleLinear()
  .domain(xExtent)
  .range([-visibleWidth / 1.75, visibleWidth / 1.75]);
const zScale = d3
  .scaleLinear()
  .domain(zExtent)
...
const result = sampled.map((d) => ({
  ...
  color: d.x < colorThreshold ? d3.interpolateGnBu(zNorm(d.z)) : d3.interpolateYlOrRd(xNorm(d.x)),
